...
`~/.astravia/conversation`，desktop-app 中虚拟注入的「对话」项目的**项目根 cwd**（常量 `DEFAULT_CONVERSATION_CWD`，`packages/desktop-app/src/main/ipc/fs.ts`）。session jsonl 文件集中落在 `<conversation cwd>/.astravia/sessions/`。
...
一个**全局、跨所有项目/session** 的系统提示词追加能力，入口在 desktop-app 设置页「Agent配置」最上方。由两部分组成：一个 [[人设]] 单选 + 一段 [[自定义指令]] 自由文本。配置写入 `~/.astravia/agent/settings.json` 的 `personalization` 块（`{ personaId, customPrompt }`），与 [[image budget]] 的 `maxRecentImages` 同文件不同字段。
...
来源分层：每个 `api` 类型在 `@astravia/ai` 内置一份**预设档位列表**作为「新建模型时的预填 + 空列表 fallback」；但**只是预设、非约束**——模型可自由改写自己的档位列表（[[预设服务商]]与手搓 provider 均走 desktop 本地配置）。列表为空时 fallback 到该 api 预设。
...
客户端 `~/.astravia/skills-manifest.json`，记录每个已装 Skill 的安装态（`version`/`source`/`enabled`/`alias`/`marketDescription`）。本地文件扁平铺在 `~/.astravia/skills/<name>/`，**同名同时只存一个版本**，路径不含版本号。manifest 里的 `version` 是 [[skill version]] 的更新比对基准。
...
同名碰撞时 **Astravia 专属优先于通用**：通用 Agent Skill 目录在所有 Astravia 原生来源（`user`/`project`）之后加载，先加载者胜，故加载顺序为 `user → project → agents-user → agents-project`——通用 Agent Skill 是补充而非覆盖内置。两处目录均纳入 agent 路径保护（只读、禁止 agent 新增/修改），与 Astravia 自家 skill 目录同等对待。
...
desktop-app 主进程注册的通用静态文件协议：`astravia-file://local/<绝对路径>`，**pathname 承载路径**（区别于 [[媒体流协议]] 的 query 参数形态），故 HTML 内的相对资源（css/js/图片）能按目录正确解析；mime 按扩展名映射常见 web 资源，路径校验复用预览沙箱（项目根/主目录内可读）。动机：iframe `srcDoc` 无法加载相对资源，凡需「整页带资源地预览项目内 HTML」走本协议。见 ADR-0027。
...
随 App 一起发布、**用户不可删除/修改**的[[可信插件]]，与用户自行安装的插件（`source: "archive" | "remote"`）相对，来源标记 `source: "system"`。物理上从**只读位置直服**——打包后在 `process.resourcesPath/system-plugins/<id>/`，dev 下在 `packages/plugins/presets/<id>/`——`astravia-plugin://` 解析按 source 选 base 目录，**不**拷进 `~/.astravia`、**不**写进 `plugins-manifest.json`（该文件只存用户态：用户插件记录 + 用户对系统插件的偏好覆盖）。`listPlugins()` 时由运行时发现并与用户插件合并呈现。因随 App 发布，版本跟随 App，不走用户插件的 `availableVersion / pendingVersion` 更新流。
...
desktop-app 主进程注册的自定义 protocol（`astravia-media://`），把校验过的本地媒体路径映射为支持 Range 的流式 URL，供 `<audio>`（未来含 `<video>`）直接作 `src`。与既有预览的 `readFile` IPC + base64 全量加载**并存**：图片/pdf/docx 等小文件维持旧路径，只有音视频走本协议。见 ADR-0021。
...
desktop-app 主进程的图像 IPC 服务：读[[插件设置]]拿 endpoint/模型/key，调用 OpenAI `/v1/images`（生成 / 编辑），把图像字节按 session 落盘，返回引用 id + `astravia-media://` URL。两条入口共用它：生成走 coding-agent 内置 image tool（薄包装转调，tool 通过 host 注入拿到该服务句柄，因 coding-agent 不能依赖 desktop-app）；[[图改图]]面板编辑走插件经 SDK 直调。是「一份实现、两条入口」的单一真相源。
...
**统一只发生在「数据存放」与「概念呈现」两层，刻意不统一物理分发与安装**：skill 装 `~/.astravia/skills/`、plugin 装 `~/.astravia/plugins/`、MCP 写进 `~/.astravia/agent/mcp.json` 的一个 key ——三条安装轨道原样保留，因为它们本来就是三种不同的运行时机制。
...
`@astravia/capability-sdk` / `@astravia/capability-runtime` 里的权限契约层：Capability ID、Grant、access session、constraint、audit（见 `docs/capabilities/README.md`）。回答的是「某个 subject 能否调用某个宿主能力出口」。
...
工作态 [[.vetd（Astravia Design 文件）]] 旁与之命名约定绑定的目录（`x.vetd` ↔ `x.vetd.d/`），存放设计的**纯源码**：各 [[Frame（设计画框）]] 的 TSX 文件与共享 `theme.css`（Tailwind v4 `@theme` 令牌，即统一色彩系统）。**不含 node_modules、不含构建配置**——依赖与工具链属于 [[共享引擎（design engine）]]。它是 agent 编辑的落点；与清单脱钩（单独移动其一）是已知代价，靠命名约定与导出命令缓解。
