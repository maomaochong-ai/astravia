...
- **宿主共享 `@astravia/ui`**：`astraviaPluginFederation` 默认将 `@astravia/ui` 设为 MF `singleton + import:false`，并 rollup external 到 `astravia-host://ui`，与 desktop-app 的 share scope / host shim 对齐；插件可选用宿主 primitives 而不打进 bundle。
