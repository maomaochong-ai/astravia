...
import { parsePackagedAstd } from "../src/export/import-design";
...
describe("老分享包", () => {
	it("没有 history.zip 时解析成 historyZip: null，其余照常", () => {
		const packaged = parsePackagedAstd(legacyPackage());
	...
	it("带 history.zip 的新包解析得出历史", () => {
		...
		expect(parsePackagedAstd(withHistory).historyZip).not.toBeNull();
	});
});
