...
import { checkSources } from "../src/astd/check-sources";
...
it("points a missing package at astd_install", () => {
	...
	expect(issue?.message).toContain("astd_install");
...
it("never suggests installing an icon package", () => {
	...
	expect(issue?.message).not.toContain("astd_install");
	expect(issue?.message).toContain("icon-[lucide--search]");
});
