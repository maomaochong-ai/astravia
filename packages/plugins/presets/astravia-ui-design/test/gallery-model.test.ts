...
function design(name: string, modifiedAt: number): GalleryDesign {
	return { astdPath: `/w/p/${name}.astd`, name, modifiedAt };
}
...
describe("scanProjectDesigns", () => {
	it("只认根目录一层里的 .astd 目录", async () => {
		const fs = fsStub([
			{ name: "a.astd", isDirectory: true },
	...
	it("时间取 design.json 的 mtime，缺失时退回目录 mtime", async () => {
		const fs = fsStub(
			[
				{ name: "a.astd", isDirectory: true, modifiedAt: 100 },
				{ name: "b.astd", isDirectory: true, modifiedAt: 900 },
			],
			{ "/w/p/a.astd/design.json": 5000 },
		);
		const designs = await scanProjectDesigns(fs, "/w/p");
...
describe("scanProjectDesigns 的旧格式迁移", () => {
	it("扫描时就地把 v1（.vetd 文件 + .vetd.d/ 目录）升级成设计包并列出来", async () => {
		...
		expect(designs.map((item) => item.astdPath)).toEqual(["/w/p/app.astd"]);
		expect(files.get("/w/p/app.astd/design.json")).toBe(MANIFEST);
		expect(files.get("/w/p/app.astd/frames/home.tsx")).toBe("export default null;");
	...
	it("同名设计包已经存在时不碰残留的旁挂目录（rename 会撞上已存在的目标）", async () => {
		const { fs, files } = migratableFs({
			"/w/p/app.astd/design.json": MANIFEST,
