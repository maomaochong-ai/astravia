/**
 * astd_history / astd_restore 的对外合同：模型拿到什么、挑错版本时能不能自己改正。
 */
...
beforeEach(() => {
	...
	registerHistoryTools(ctx, {
		resolveAstdPath: async (_host, _cwd, explicit) => explicit ?? "/w/a.vetd",
		scopeUse: ["project", "conversation"],
		agentMode: ["work"],
	});
});

describe("astd_history", () => {
	it("倒序返回版本，并标出哪一条是当前内容", async () => {
		const result = await call("astd_history");
	...
	it("解析不到设计时报错而不是返回空列表", async () => {
		registerHistoryTools(ctx, {
			resolveAstdPath: async () => {
				throw new Error("No .vetd design document in the workspace.");
			},
			scopeUse: ["project"],
			agentMode: ["work"],
		});
		expect(await call("astd_history")).toMatchObject({ ok: false });
	});
});

describe("astd_restore", () => {
	it("没给 version 就直接拒绝", async () => {
		expect(await call("astd_restore")).toMatchObject({ ok: false });
		expect(restoreDesign).not.toHaveBeenCalled();
	});

	it("版本 id 不在历史里时让它先查一次，而不是硬试", async () => {
		const result = await call("astd_restore", { version: "不存在" });
		expect(result.ok).toBe(false);
		expect(String(result.error)).toContain("astd_history");
		expect(restoreDesign).not.toHaveBeenCalled();
	});

	it("恢复成功时报出「回去的路」——挑错版本能自己改正", async () => {
		const result = await call("astd_restore", { version: "c1" });
	...
	it("内容与目标版本一致时如实说无事可做", async () => {
		...
		expect(await call("astd_restore", { version: "c1" })).toMatchObject({ ok: true, unchanged: true });
	});
	...
	it("重装了依赖会报出来", async () => {
		...
		expect(await call("astd_restore", { version: "c1" })).toMatchObject({ dependenciesReinstalled: true });
	});
});
