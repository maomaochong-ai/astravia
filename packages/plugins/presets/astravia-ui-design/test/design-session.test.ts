...
import { DesignSession } from "../src/astd/design-session";
...
it("agent 手写的、缺 meta 的 manifest 不会让 open 崩掉", async () => {
	...
	const { ctx, fs, written } = fakeCtx({
		[`${VETD}/design.json`]: JSON.stringify(manifest),
		[`${VETD}/frames/index.tsx`]: frameSource("首页"),
	});
	...
	expect(written[`${VETD}/design.json`]).toBeDefined();
...
it("agent 自创 schema 写的 manifest：条目丢弃后由 tsx 全量重建，一帧不少", async () => {
	...
	const { ctx } = fakeCtx({
		[`${VETD}/design.json`]: JSON.stringify(manifest),
		[`${VETD}/frames/index.tsx`]: frameSource("首页"),
		[`${VETD}/frames/search.tsx`]: frameSource("找车位"),
	});
	const session = new DesignSession(ctx, VETD);
