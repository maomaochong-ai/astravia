...
it("keeps the overlay alive through a long generation", () => {
	...
	const overlay = host.querySelector(".astd-activity-overlay") as HTMLElement | null;
...
it("renders the overlay with the per-kind decorations", () => {
	...
	expect(overlay?.style.opacity).toBe("1");
	expect(host.querySelector(".astd-fluid-blob")).not.toBeNull();
	expect(host.querySelector(".astd-scan-beam")).not.toBeNull();
	expect(host.querySelector(".astd-bot-think")).not.toBeNull();

	act(() => {
		root.render(<FrameActivityOverlay activity="creating" />);
	});
	expect(host.querySelectorAll(".astd-spark").length).toBe(4);
