...
import type { DesignSession } from "../src/astd/design-session";
...
const reload = vi.fn();
const session = { dirPath: "/w/a.astd", reload } as unknown as DesignSession;
const MARKER = "/w/a.astd/.history/peek.json";
...
describe("enterPeek", () => {
	...
	it("标记里记着要退回哪一版", async () => {
		...
		expect(await readPeekState(ctx, "/w/a.astd")).toEqual(state);
	});
	...
	it("只有 design.json 变了不落版本——那是拖画框/平移视口，不值得占一条历史", async () => {
		...
		expect(await readPeekState(ctx, "/w/a.astd")).toMatchObject({ manifest: expect.any(String) });
	});
	...
	it("工作区干净时既不提交也不存 manifest", async () => {
		...
		await enterPeek(ctx, session, target);
		expect(commitHistory).not.toHaveBeenCalled();
		expect((await readPeekState(ctx, "/w/a.astd"))?.manifest).toBeUndefined();
	});
...
describe("exitPeek", () => {
	it("写回进入时的那一版，再删标记", async () => {
		...
		expect(runHistoryCommand).toHaveBeenLastCalledWith(ctx, { cmd: "checkout", dir: "/w/a.astd", sha: "head" });
...
describe("查看期间的自动提交", () => {
	it("被跳过——否则回合结束会把旧版本记成新版本", async () => {
		...
		const listFilesRecursive = async () => [
			{ name: "design.json", path: "/w/a.astd/design.json", relPath: "a.astd/design.json" },
		];
