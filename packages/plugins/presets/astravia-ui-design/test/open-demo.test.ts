...
describe("demoTempFileName", () => {
	it("只保留 slug 字符，防止拼进临时路径的名字带穿越", () => {
		expect(demoTempFileName("linear")).toBe("astravia-demo-linear.html");
		expect(demoTempFileName("../..//etc")).toBe("astravia-demo-etc.html");
		expect(demoTempFileName("!!!")).toBe("astravia-demo-demo.html");
	});
});
