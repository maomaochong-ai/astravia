/**
 * v1（`x.vetd` 文件 + `x.vetd.d/` 目录）→ v2 设计包（`x.astd/` 目录）的就地迁移。
 *
...
import { findAstdFiles } from "../src/astd/discover";
import { migrateLegacyDesign } from "../src/astd/migrate";
...
describe("migrateLegacyDesign", () => {
	it("把 manifest 收进 .astd 目录，让目录顶替旧格式的位置", async () => {
		...
		expect(await migrateLegacyDesign(fs, "/proj/app.astd")).toBe(true);

		expect(files.get("/proj/app.astd/design.json")).toBe(MANIFEST);
		expect(files.get("/proj/app.astd/frames/home.tsx")).toBe("export default null;");
		expect(files.get("/proj/app.astd/theme.css")).toBe(":root{}");
		// 旧的两个条目都不复存在——留着任何一个都会让下一次扫描再迁移一遍。
		expect([...files.keys()].some((path) => path.includes(".vetd.d/"))).toBe(false);
		expect([...files.keys()].some((path) => path.endsWith("/app.vetd"))).toBe(false);
		expect(await fs.readFile("/proj/app.astd/design.json").then(() => "file", () => "dir")).toBe("file");
	});

	it("没有源码目录的空设计也能迁移，全程不删内容", async () => {
		const { fs, files } = fakeFs({ "/proj/empty.vetd": MANIFEST });

		expect(await migrateLegacyDesign(fs, "/proj/empty.astd")).toBe(true);

		expect(files.get("/proj/empty.astd/design.json")).toBe(MANIFEST);
		expect(files.has("/proj/empty.astd.migrating")).toBe(false);
	});
	...
	it("既没有旧文件也没有旧目录时什么都不做", async () => {
		...
		expect(await migrateLegacyDesign(fs, "/proj/ghost.astd")).toBe(false);
		expect([...files.keys()]).toEqual(["/proj/other.txt"]);
	});

	it("已经是设计包时什么都不做", async () => {
		const { fs, files } = fakeFs({ "/proj/app.astd/design.json": MANIFEST });
		expect(await migrateLegacyDesign(fs, "/proj/app.astd")).toBe(false);
		expect(files.get("/proj/app.astd/design.json")).toBe(MANIFEST);
	});

	it("打包分享文件不是设计，不迁移", async () => {
		const { fs, files } = fakeFs({ "/proj/app-share.vetd": "PK\u0004binary" });
		expect(await migrateLegacyDesign(fs, "/proj/app-share.astd")).toBe(false);
		expect(files.get("/proj/app-share.vetd")).toBe("PK\u0004binary");
	});
});

describe("findAstdFiles", () => {
	it("同时列出设计包与刚被迁移的旧设计，并排除分享文件", async () => {
		const { fs } = fakeFs({
			"/proj/new.astd/design.json": MANIFEST,
		...
		expect(await findAstdFiles(fs, "/proj")).toEqual(["/proj/new.astd", "/proj/old.astd"]);
	});
	...
	it("迁移停在「旧 manifest 已删、目录还没改名」时仍能找到并救回这份设计", async () => {
		...
		expect(await findAstdFiles(fs, "/proj")).toEqual(["/proj/app.astd"]);
		expect(files.get("/proj/app.astd/frames/home.tsx")).toBe("export default null;");
	});
	...
	it("同名设计包已经存在时，残留的旁挂目录不被当成另一份设计", async () => {
		const { fs, files } = fakeFs({
			"/proj/app.astd/design.json": MANIFEST,
			"/proj/app.vetd.d/design.json": MANIFEST,
		});
