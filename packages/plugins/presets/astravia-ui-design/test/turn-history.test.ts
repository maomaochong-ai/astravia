...
describe("hook 接线", () => {
	...
	it("Stop 用这一轮用户那句话当标题", async () => {
		...
		expect(commitHistory).toHaveBeenLastCalledWith(ctx, "/w/a.astd", "把导航栏改到左侧", { skipManifestOnly: true });
	});
	...
	it("新一轮开始前先封存上一轮被中断的改动", async () => {
		...
		expect(commitHistory).toHaveBeenLastCalledWith(expect.anything(), "/w/a.astd", "把导航栏改到左侧（未完成）", {
	...
	it("会话结束后不再拿旧标题", async () => {
		...
		await hooks.get("Stop")?.handler({ session, event: {} });
		expect(commitHistory).toHaveBeenLastCalledWith(expect.anything(), "/w/a.astd", "更新设计", { skipManifestOnly: true });
	});
});
