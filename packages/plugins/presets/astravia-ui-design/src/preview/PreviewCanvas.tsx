...
function boundsOfFrames(frames: readonly AstdFrameEntry[]): Rect | null {
...
function SnapshotFrame({ html, frameId, onLoaded }: { html: string; frameId: string; onLoaded(): void }) {
	...
	return (
		<iframe
			...
			onLoad={() => {
				ref.current?.contentWindow?.postMessage({ astd: true, type: "show-frame", id: frameId }, "*");
				onLoaded();
			}}
		/>
	);
...
export function PreviewCanvas({ manifest, snapshotHtml }: PreviewCanvasProps) {
	...
	return (
		<div
			ref={containerRef}
			className="relative h-full w-full cursor-grab select-none overflow-hidden astd-canvas-bg active:cursor-grabbing"
				...
				{renderedFrames.map((frame) => (
					...
					>
						<div
							...
							style={{ transform: "scale(var(--astd-lscale, 1))", transformOrigin: "0 100%" }}
						>
