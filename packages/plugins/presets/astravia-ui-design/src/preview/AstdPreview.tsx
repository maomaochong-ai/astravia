import type { PluginPreviewFile } from "@astravia-org/plugin-sdk";
import { useTranslation } from "@astravia-org/plugin-sdk";
import { useEffect, useState } from "react";
import { parsePackagedAstd, importPackagedAstd, type PackagedAstd } from "../export/import-design";
...
import { emptyManifest, type AstdManifest } from "../astd/manifest-types";

type PreviewState =
	| { kind: "loading" }
	| { kind: "invalid" }
	| { kind: "working"; manifest: AstdManifest }
	/** `snapshotHtml` 是已经把包内资源内嵌进去的版本，见 snapshot-assets。 */
	| { kind: "packaged"; manifest: AstdManifest; packaged: PackagedAstd; snapshotHtml: string | null };

function parseManifest(json: string): AstdManifest | null {
	try {
		const parsed = JSON.parse(json) as AstdManifest;
...
export function AstdPreview({ file }: { file: PluginPreviewFile }) {
	...
	useEffect(() => {
		...
		const load = async (): Promise<void> => {
			...
			if (bytes[0] === 0x50 && bytes[1] === 0x4b) {
				const packaged = parsePackagedAstd(bytes);
	...
	const runImport = async (packaged: PackagedAstd): Promise<void> => {
		try {
			const ctx = getPluginCtx();
			if (!file.path) throw new Error("no file path");
			const targetDir = file.path.slice(0, file.path.lastIndexOf("/"));
			const astdPath = await importPackagedAstd(ctx, packaged, targetDir, file.name);
			notify({ message: t("canvas.import.done", { path: astdPath }), variant: "success", durationMs: 5000 });
