...
import { toAstraviaFileUrl } from "./file-url";
...
function CaptureSkeleton() {
	...
	return (
		<div
			className={`astd-shot-pulse flex w-24 shrink-0 items-center justify-center rounded-lg border border-border ${ITEM_HEIGHT}`}
...
export function ScreenshotSwiper({
	...
	const { t } = useTranslation();
	const group = snapshots.map((snapshot) => ({ id: snapshot.path, url: toAstraviaFileUrl(snapshot.path) }));
