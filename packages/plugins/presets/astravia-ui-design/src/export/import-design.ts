...
export function parsePackagedAstd(bytes: Uint8Array): PackagedAstd {
...
/**
 * Unpack a share file into a working design bundle (`x.astd/`) inside
 * `targetDir`. Returns the new bundle path.
 */
export async function importPackagedAstd(
	ctx: PluginContext,
	packaged: PackagedAstd,
	targetDir: string,
	sourceFileName: string,
): Promise<string> {
	const baseName = sanitizeDesignName(sourceFileName.replace(/\.(?:vetdz|vetd)$/i, "").replace(/-share$/i, ""));
	let astdPath = `${targetDir}/${baseName}.astd`;
	let suffix = 1;
	while ((await ctx.fs.stat(astdPath)) !== null) {
		astdPath = `${targetDir}/${baseName}-${suffix}.astd`;
		suffix += 1;
	}
	for (const [rel, bytes] of packaged.designFiles) {
		await ctx.fs.writeFile(`${astdPath}/${rel}`, base64FromBytes(bytes), "base64");
	}
	await ctx.fs.writeFile(manifestPathOf(astdPath), packaged.manifestJson);
	// 历史在源码之后还原：`.history/` 里存的就是这些文件的旧版本，先落地内容再接上历史，
	// 之后第一次打开这份设计时不会因为「工作区与 HEAD 不一致」而多出一个空版本。
	if (packaged.historyZip) {
		await unpackHistoryFromShare(ctx, astdPath, base64FromBytes(packaged.historyZip));
	}
	return astdPath;
}
