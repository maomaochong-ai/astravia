 ...
 * 上画布，`frame-size-missing` 照常从 checkSources 报出去，随 astd_screenshot /
 * astd_status 的 `issues` 交到 agent 手上。agent 该补的声明一条都没少，只是补之前
 * 画布不再是空的。
 */
 ...
 * `defaultFrameSize`，走的就是这一条；经 astd_create 建的设计都带着品类，轮不到
