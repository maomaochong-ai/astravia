/**
 * 一份设计的依赖清单：`x.astd/package.json`（ADR-0068）。
 *
 * 唯一事实源，三个消费方问的是同一件事：
 * - scaffold：新建设计时写出清单，让 `x.astd/` 从第一秒就是一个可 npm install 的工程。
 * - check-sources：`uninstalled-import` 判据里「这份设计装了什么」的那一半。
 * - engine-manager：打开设计时决定要不要先补装，以及 astd_install 落到哪个目录。
 *
 * 事实源是**声明**而不是 node_modules：清单在、依赖没装是一个正常且短暂的状态
 * （刚从 .astdz 导入），由补装接住；反过来 node_modules 里有什么则完全不作数。
 */
...
/** 依赖装在哪。生成物，不进 .astdz（见 bundle-paths 的 GENERATED_PREFIXES）。 */
 ...
 *
 * 判据是「声明了依赖，但 node_modules 不在」——这正是从 .astdz 导入、或者从别人的
