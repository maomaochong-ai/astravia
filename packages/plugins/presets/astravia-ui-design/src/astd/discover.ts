...
const BUNDLE_MANIFEST_SUFFIX = `.astd/${MANIFEST_FILE}`;
/**
...
function insideDesign(relPath: string): boolean {
	return relPath.includes(".astd/") || relPath.includes(".vetd.d/");
}
 ...
 */
export async function findAstdFiles(fs: PluginFsApi, cwd: string): Promise<string[]> {
	...
	for (const legacy of picked.legacyFiles) {
		const migrated = await migrateLegacyDesign(fs, toAstdPath(legacy)).catch(() => false);
		if (migrated) found.push(toAstdPath(legacy));
	}
	return found.sort();
}
