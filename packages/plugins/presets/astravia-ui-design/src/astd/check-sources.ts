 ...
 * 结果经 astd_status 的 `issues` 返回，agent 本来就会反复调那个工具。
 *
 ...
 * （ADR-0068：包装在 x.astd/node_modules/ 里，各设计不同），所以整张表不能再是
...
function rulesFor(installedPackages: ReadonlySet<string>): Rule[] {
	...
	},
	{
		...
		message: (line) => {
			...
			return ICON_PACKAGES.has(pkg)
				...
				: `${base} Either install it (astd_install with packages: ["${pkg}"]) if it genuinely earns its place, or build it from what is already here: Tailwind utilities for layout/visuals, an Iconify class for a glyph, plain React state for behaviour.`;
		},
	...
	},
	{
		...
		message: `Line is over ${MAX_LINE_LENGTH} characters — the source is written as one long line. Element→source mapping is per line, so every element then reports the same location and the user's "让 Astravia 调整" can no longer target anything. Format it normally, one element per line for nested markup.`,
	},
	];
}
...
function checkFrameSize(file: SourceFile, others: readonly SourceFile[]): SourceIssue | null {
	...
	const reference =
		known.length > 0
			...
			: " Pick the size from the product type (see the astravia-ui-design skill).";
	return {
		file: file.path,
		line: null,
 ...
 * astd_status 的返回撑爆，而 agent 需要的信息第一条就给全了。
 */
