...
/**
 * One open design bundle (`x.astd/`). Owns the manifest (single writer): the
...
export class DesignSession {
	...
	readonly astdPath: string;
	readonly dirPath: string;
	readonly manifestPath: string;
	readonly name: string;
	manifest: AstdManifest = emptyManifest();
	...
	constructor(ctx: PluginContext, astdPath: string) {
		this.ctx = ctx;
		this.astdPath = astdPath;
		this.dirPath = astdPath;
		this.manifestPath = manifestPathOf(astdPath);
		this.name = designNameOf(astdPath);
	}
	...
	private async loadManifest(): Promise<void> {
		try {
			...
			const parsed = JSON.parse(raw.content) as AstdManifest;
			if (parsed && parsed.type === "astravia-design" && Array.isArray(parsed.frames)) {
				this.manifest = {
					...emptyManifest(),
					...parsed,
	...
	async reconcile(): Promise<void> {
		...
		const nextFrames: AstdFrameEntry[] = [];
		...
		for (const { file, id, parsed } of parsedFiles) {
			...
			known.delete(id);
			// 坐标是画布元数据，agent 手写的 manifest（旧格式、缺字段）可能没有 x/y：
			// 不能把 undefined 带进画布——像新帧一样重新摆放，别叠在 (0,0)。
			const hasPosition = Number.isFinite(existing.x) && Number.isFinite(existing.y);
			if (!hasPosition) {
				const placement = this.pendingPlacements.get(id) ?? this.autoPlacement(nextFrames);
				this.pendingPlacements.delete(id);
				nextFrames.push({
				...
				dirty = true;
			} else if (!sameMeta(meta, existing.meta)) {
	...
	private autoPlacement(placed: AstdFrameEntry[]): { x: number; y: number } {
		// 手写 manifest 的坏条目（缺 x/y）不该参与排布——undefined 会算出 NaN 并传染整个 max。
		const all = [...this.manifest.frames, ...placed].filter(
			(frame) => Number.isFinite(frame.x) && Number.isFinite(frame.y) && Number.isFinite(frame.width),
		);
	...
	updateFramePlacement(id: string, patch: Partial<Pick<AstdFrameEntry, "x" | "y" | "width" | "height">>): void {
		let changed = false;
		this.manifest = {
			...this.manifest,
	...
	updateFramePlacements(
		patches: ReadonlyMap<string, Partial<Pick<AstdFrameEntry, "x" | "y" | "width" | "height">>>,
	): void {
	...
	saveViewport(viewport: AstdCanvasViewport): void {
	...
	private persist(): Promise<void> {
		...
		this.writing = this.writing
			...
			.catch((error: unknown) => {
				console.error("astd manifest write failed", error);
			});
		return this.writing;
	}
}
