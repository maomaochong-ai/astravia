...
import { ensureDesignIgnored } from "./astd/design-ignore";
...
import type { SourceIssue } from "./astd/check-sources";
import { blockingSyntaxIssues, SYNTAX_RULE } from "./astd/check-syntax";
import { readDesignDependencies } from "./astd/design-package";
import { findAstdFiles } from "./astd/discover";
import { inspectIssues } from "./astd/inspect";
import { layoutIssues } from "./astd/layout-probe";
import { PRODUCT_SIZE_SUMMARY, PRODUCT_TYPES, resolveDefaultFrameSize } from "./astd/product-size";
 ...
 * 什么，于是每个 frame 里各抄一份导航栏。把它随 astd_status 一起端到眼前，比在
...
/**
 * astd_status 的一句话提示。按「此刻最该做的一件事」给，不叠加——三条并列的建议
 * 等于没有重点。
 */
function statusNote(
	...
	if (frameCount > 1 && shell && !shell.layout && shell.components.length === 0) {
		return "Multiple screens but NO shared UI yet. If they share a nav bar / sidebar / tab bar, extract it into components/ (or frames/_layout.tsx when it must survive navigation) rather than repeating it per frame. The structure checklist is in the astravia-ui-design skill, under Workflow.";
	}
...
export function registerDesignTools(ctx: PluginContext): void {
	ctx.agent.registerTool<CreateInput>({
		...
		handler: async ({ host, session, trigger }) => {
			...
			setPendingDesignPath(result.astdPath);
			ctx.ui.setActivityTabVisible(CANVAS_TAB_ID, true);
			ctx.ui.openActivityTab(CANVAS_TAB_ID, { width: "max" });
			return {
				ok: true,
				astdPath: result.astdPath,
				sourcesDir: result.dirPath,
				defaultFrameSize,
				note: `Design created and opened on the canvas, with NO frames yet. Its default size is ${defaultFrameSize.width}x${defaultFrameSize.height} — still declare it per frame: every frames/<id>.tsx starts with \`export const frame = { width: ${defaultFrameSize.width}, height: ${defaultFrameSize.height}, title }\`, and a screen of a different product type declares its own. Never edit design.json. More than one screen? Write the shared chrome FIRST (components/, or frames/_layout.tsx), then every frame as a short skeleton so the whole set reaches the canvas immediately, and only then fill them in one at a time — see the astravia-ui-design skill.`,
			};
		},
	});
	...
	ctx.agent.registerTool<ScreenshotInput>({
		...
		handler: async ({ host, trigger }) => {
			...
			// 风格违规一起带回去——截图本来就是每帧必调的，agent 不用再单独跑 astd_status。
			...
			if (!known.includes(frameId)) {
				// 漏声明尺寸不再让画框掉出画布（astd/frame-size.ts），所以源码在、画布上
			...
			let dataUrl: string;
			// 布局机检搭在出图这一次渲染上（见 astd/layout-probe）。走离屏窗口时页面
			...
			try {
				...
				return {
					...
					error: late
						...
						: `Screenshot failed: ${reason}. Call astd_status for design-engine diagnostics and recent build output.`,
				};
			}
			...
			const { dirPath, astdPath } = controller.session;
			...
			return {
				...
				...(framePendingNotes > 0
					? {
							...
							notesReminder: `This frame has ${framePendingNotes} pending user note(s). Call astd_notes to see them (annotated screenshot + source anchors) and address them before you finish.`,
						}
					: {}),
				...
				cards: [screenshotCardDescriptor(astdPath, dirPath, frameId)],
			};
		},
	});
	...
	ctx.agent.registerTool({
		id: "astd-status",
		name: "astd_status",
		label: "%tool.astd_status%",
		description:
			"Inspect the Astravia UI Design state: workspace designs, the design open on the canvas, its frames (id/size/title/`buildError`), its `sharedShell` (existing _layout.tsx + components/ to reuse), `issues` (files that do not parse, plus rule violations found in your sources) and engine diagnostics. Call it ONCE before editing an existing design, to learn what is already there. Afterwards you do not need it for `issues` — astd_screenshot returns them per frame.\nDo NOT use to survey a code repository, locate its UI source files or read its build state — use the ordinary file search and read tools instead.\nOnly for .astd design documents and the design canvas.",
		parameters: { type: "object", properties: {}, additionalProperties: false },
		scope_use: SCOPE_USE,
		handler: async ({ host, session }) => {
			const designs = await findAstdFiles(host.fs, session.cwd);
			const controller = getCanvasController();
			...
			return {
				...
				open: controller
					? {
							astdPath: controller.session.astdPath,
							...
							// 先掂量能不能用 Tailwind + React 写出来，真需要才 astd_install。
							...(dependencies.length > 0 ? { dependencies } : {}),
							issues,
							// 用户在画布上留的待处理备注数。非零时先调 astd_notes 看内容。
	...
	/** 无画布场景下解析目标 .astd：显式参数 > 打开的画布 > cwd 里唯一那份。 */
	const resolveAstdPath = async (host: { fs: PluginContext["fs"] }, cwd: string, explicit?: string) => {
		if (explicit) return explicit;
		const controller = getCanvasController();
		if (controller) return controller.session.astdPath;
		const designs = await findAstdFiles(host.fs, cwd);
		if (designs.length === 1) return designs[0];
		throw new Error(
			designs.length === 0
				? "No .astd design document in the workspace. Create one with astd_create first."
	...
	ctx.agent.registerTool<InstallInput>({
		...
		parameters: {
			...
			properties: {
				...
				design: {
					type: "string",
					description: "Path to the `x.astd/` directory (default: the design open on the canvas).",
				},
			},
			required: ["packages"],
			additionalProperties: false,
		...
		scope_use: SCOPE_USE,
		// 往设计工程的 node_modules 写依赖树（落在用户工作区的 .astd 目录内）。
		handler: async ({ host, session, trigger }) => {
			...
			const designDir = await resolveAstdPath(host, session.cwd, trigger.input.design);
			try {
	...
	registerHistoryTools(ctx, { resolveAstdPath, scopeUse: SCOPE_USE });
	...
	ctx.agent.registerTool<NotesInput>({
		...
		handler: async ({ host, session, trigger }) => {
			...
			if (resolve && resolve.length > 0) {
				...
				let astdPath: string;
				try {
					astdPath = await resolveAstdPath(host, session.cwd);
				} catch (error) {
					return { ok: false, error: error instanceof Error ? error.message : String(error) };
				}
				const dirPath = astdPath;
