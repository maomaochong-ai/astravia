...
export function registerDesignTools(ctx: PluginContext): void {
	ctx.agent.registerTool<CreateInput>({
		...
		description:
			"Create a new Astravia UI Design document (.vetd manifest + sidecar sources) in the current workspace and open it on the design canvas. Use when the user asks to start a UI design / mockup. After creating, add frames by writing frames/<id>.tsx files — never edit the .vetd manifest itself.",
		parameters: {
			type: "object",
			properties: {
				name: {
	...
	ctx.agent.registerTool({
		...
		description:
			"Inspect the Astravia UI Design state: design documents in the workspace, the design open on the canvas, its frames (id/size/title) and design-engine diagnostics (dev-server liveness + recent build output). Use to find frame ids, or to diagnose why the canvas shows a build error.",
