...
/**
 * 左侧备注抽屉：待处理/已处理两段、让 Astravia 处理（全部/单条）、清空已处理。
...
export function NotesDrawer({ store, session, cwd, onLocate, offsetTop, onClose }: NotesDrawerProps) {
	...
	return (
		...
		<div
			className="astd-note astd-note-drawer-enter astd-note-surface pointer-events-auto absolute z-40 flex flex-col overflow-hidden rounded-2xl border border-border/70 bg-popover/95 shadow-2xl ring-1 ring-black/5 backdrop-blur-xl"
		...
		>
			<header className="flex shrink-0 items-center gap-2 border-b border-border/60 px-3 py-2.5">
				...
				{pending.length > 0 ? (
					<span
						...
						style={{ background: "var(--astd-note-pending, #2563eb)" }}
					>
						{pending.length}
					</span>
				) : null}
			...
			<div
				ref={scrollRef}
				className={`astd-note-scroll min-h-0 flex-1 overflow-y-auto ${fadeBottom ? "astd-note-scroll-fade" : ""}`}
			>
				...
				<span className="min-w-0 flex-1">
					...
					{lastAgent ? (
						<span className="mt-1 flex items-start gap-1 text-[10px] leading-snug text-muted-foreground">
							<span className="shrink-0 font-medium">Astravia</span>
