 ...
 * (astd_screenshot / astd_status need to reach the live canvas), plus the
 * "modifying" frame state fed by conversation tool events (index.tsx).
 */
import type { NotesStore } from "../notes/notes-store";
import { classifySource } from "../astd/bundle-paths";
...
export interface CanvasController {
	...
	/** 当前打开设计的备注（astd_notes / astd_status 都从这里读写）。 */
	notes: NotesStore;
	port: number;
	...
	openDesign(astdPath: string): void;
}
...
/** Hand-off from tools (astd_create) to the canvas: open this design when the tab mounts/refreshes. */
let pendingDesignPath: string | null = null;

export function setPendingDesignPath(astdPath: string | null): void {
	pendingDesignPath = astdPath;
}
 ...
 * 判定规则与画布的位图刷新共用（见 astd/bundle-paths），不在这里另立一套。
 */
...
/**
 * frameId → 编译/渲染错误。放在模块级而不是画布 state 里：agent 工具（astd_screenshot）
