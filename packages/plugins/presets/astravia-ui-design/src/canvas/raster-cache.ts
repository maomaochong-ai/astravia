...
const DB_NAME = "astravia-ui-design";
const STORE = "rasters";
/**
 * 画廊封面：整块画布的全景图，一份设计一张，key 就是 astdPath。
 *
...
/** `${astdPath}::${frameId}`——设计文档之间天然隔离，同一份里按 frame 取。 */
function keyOf(astdPath: string, frameId: string): string {
	return `${astdPath}::${frameId}`;
}
		...
		(db) =>
			new Promise<T | null>((resolve) => {
				...
				if (!db.objectStoreNames.contains(storeName)) {
					console.error(
						`[astravia-ui-design] IndexedDB store "${storeName}" is missing (db v${db.version}); bump DB_VERSION so the upgrade runs.`,
					);
					return resolve(null);
				}
				try {
...
export async function loadRasters(
	...
	await Promise.all(
		frameIds.map(async (frameId) => {
			const value = await runStore<unknown>("readonly", (store) => store.get(keyOf(astdPath, frameId)));
...
export async function saveRaster(astdPath: string, frameId: string, dataUrl: string): Promise<void> {
	await runStore("readwrite", (store) => store.put(dataUrl, keyOf(astdPath, frameId)));
}
 ...
 *
 * 整份设计稿被删除的情况不处理——没有全局索引就无从知道哪些 astdPath 已经不在了，
 * 为几百 KB 的条目维护一张索引不划算。
 */
export async function pruneRasters(astdPath: string, keep: readonly string[]): Promise<void> {
	const alive = new Set(keep.map((frameId) => keyOf(astdPath, frameId)));
	const prefix = `${astdPath}::`;
...
export async function loadCover(astdPath: string): Promise<string | null> {
	const value = await runStore<unknown>("readonly", (store) => store.get(astdPath), COVER_STORE);
	return typeof value === "string" ? value : null;
}

export async function saveCover(astdPath: string, dataUrl: string): Promise<void> {
	await runStore("readwrite", (store) => store.put(dataUrl, astdPath), COVER_STORE);
}

export async function deleteCover(astdPath: string): Promise<void> {
	await runStore("readwrite", (store) => store.delete(astdPath), COVER_STORE);
}
