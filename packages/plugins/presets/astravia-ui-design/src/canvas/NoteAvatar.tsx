...
export function NoteAvatar({ author, size, tone = "solid" }: NoteAvatarProps) {
	...
	if (author === "agent") {
		return (
			<span
				className="astd-note-face shrink-0"
				style={{
					width: size,
					height: size,
					// 脸是白底、五官取 currentColor：放在绿色气泡里就是绿眼睛白脸。
					color: onColor ? "var(--astd-note-resolved, #10b981)" : "var(--muted-foreground, #71717a)",
	...
	return (
		<span
			...
			style={{
				...
				color: onColor ? "var(--astd-note-pending, #2563eb)" : "var(--muted-foreground, #71717a)",
			}}
			aria-hidden
		>
...
export function NotePin({ resolved, size, number }: NotePinProps) {
	return (
		<span
			className={`astd-note-pin relative flex items-center justify-center rounded-full rounded-bl-[4px] ${
				resolved ? "astd-note-pin-resolved" : ""
			}`}
		...
		>
			...
			{number !== null ? (
				<span
					className="absolute -right-1 -top-1 flex items-center justify-center rounded-full font-semibold tabular-nums text-[var(--astd-note-pending,#2563eb)]"
					style={{
