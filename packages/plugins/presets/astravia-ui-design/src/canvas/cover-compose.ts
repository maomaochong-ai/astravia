 ...
 */
export async function composeCover(astdPath: string, frames: readonly AstdFrameEntry[]): Promise<string | null> {
	if (frames.length === 0) return null;
...
export async function refreshCover(astdPath: string, frames: readonly AstdFrameEntry[]): Promise<void> {
	try {
		const dataUrl = await composeCover(astdPath, frames);
		if (dataUrl) await saveCover(astdPath, dataUrl);
	} catch {
		// 见上：封面失败不冒泡。
	}
}
