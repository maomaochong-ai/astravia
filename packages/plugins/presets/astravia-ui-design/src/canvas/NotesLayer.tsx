...
import type { AstdFrameEntry } from "../astd/manifest-types";
...
interface NotesLayerProps {
	store: NotesStore;
	frames: readonly AstdFrameEntry[];
 ...
 * （跟着画布平移缩放），自身按 `--astd-lscale` 反向缩放保持恒定视觉大小——与
...
export function NotesLayer({
	 ...
	 * 而 agent 的回复经 astd_notes 落进 store，画布这边只看得到「集合变了」。
	...
	return (
		<div className={`astd-note ${interactive ? "contents" : "pointer-events-none contents"}`}>
				...
				return (
					<button
						...
						className={`absolute z-30 cursor-pointer rounded-full rounded-bl-[4px] ${
							celebrating ? "astd-note-celebrate astd-note-ripple" : ""
					...
					>
						{/* 头像即状态：待处理是提问的人，已处理是回复过的 Astravia。 */}
		...
		<div className="flex flex-col">
			<header className="flex items-center gap-1.5 border-b border-border/60 px-2.5 py-2">
				<span
					...
					style={{
						background: pending ? "var(--astd-note-pending, #2563eb)" : "var(--astd-note-resolved, #10b981)",
					}}
					aria-hidden
				/>
...
/** 一条 thread 消息：头像 + 名字 + 气泡，Astravia 的用主题色底与之区分。 */
		...
		<div className="flex items-start gap-1.5">
			...
			<div className="min-w-0 flex-1">
				<span className="mb-0.5 block text-[10px] font-medium text-muted-foreground">
					{isAgent ? "Astravia" : t("notes.author.user")}
				</span>
				<div
