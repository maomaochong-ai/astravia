...
import { frameUrl } from "../astd/frame-url";
...
export const FrameView = memo(function FrameView({
	 ...
	 */
	const labelScale = "var(--astd-lscale, 1)";
		...
		>
				...
				{renaming ? (
					...
					<button
						...
						className={`min-w-0 cursor-pointer truncate font-medium ${
							selected ? "text-[var(--astd-selected)]" : "text-muted-foreground"
						}`}
			...
			<div
				className={`relative h-full w-full overflow-hidden ring-offset-0 ${
					selected ? "ring-2 ring-[var(--astd-selected)]" : ""
				} ${activity === "modifying" ? "astd-modifying" : ""}`}
			>
			...
			{selected && resizable
				? handles.map(({ edge, className }) => (
						...
						<div
							key={edge}
							className={`absolute z-10 size-2 rounded-full border border-[var(--astd-selected)] bg-white ${className}`}
