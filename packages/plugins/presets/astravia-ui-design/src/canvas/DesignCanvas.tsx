...
import { type AskShot, buildAskPrompt } from "./ask-astravia";
import { AskAstraviaButton } from "./AskAstraviaButton";
import { AskAstraviaPopover } from "./AskAstraviaPopover";
...
export function DesignCanvas({ session, port, bridge, captureRef, refreshRef }: DesignCanvasProps) {
	...
	/**
	 * 右击 frame 弹菜单。菜单里的动作（让 Astravia 调整 / 导出渲染图）按当前选中执行，
	...
	useEffect(() => {
		...
		const onKeyDown = (event: KeyboardEvent): void => {
			// 画布里的输入框（重命名、让 Astravia 调整）也在这个容器内，事件会冒泡上来。
	...
	/** "让 Astravia 调整"：N 张截图（DOM 选中时保留高亮）+ 元信息 + 用户建议 → sendPrompt。 */
	const sendAskToAstravia = (suggestion: string): void => {
		if (askBusy) return;
		// 关掉浮层是「已交出去」的反馈，不等截图落盘、更不等 Astravia 跑完：
