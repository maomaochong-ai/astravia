import { usePluginShortcutScope, useTranslation } from "@astravia-org/plugin-sdk";
import {
	type CSSProperties,
...
import { classifySource, isGeneratedPath, normalizeRelative } from "../astd/bundle-paths";
import type { AstdFrameEntry, AstdManifest } from "../astd/manifest-types";
import {
	type ArrangeItem,
	type GapBand,
	gapBands,
...
interface DesignCanvasProps {
	...
	notes: NotesStore;
	/** 活动面板的 cwd，备注面板的「让 Astravia 处理」会话闸口用。 */
	cwd: string | null;
	port: number;
	bridge: BridgeHub;
	/**
	 * 出口：把「先拉回活体再截」的截图函数交给 CanvasTab，astd_screenshot 用它。
	...
	previewing: boolean;
	/**
	 * 出口：astd_notes 的锚点保鲜。拉活体后按 domPath/坐标逐条查元素，
...
export function DesignCanvas({
	...
	const [manifest, setManifest] = useState<AstdManifest>(session.manifest);
	...
	const runPeekAction = async (action: () => Promise<void>): Promise<void> => {
		...
		try {
			...
			console.error("[astravia-ui-design] 查看历史版本失败", error);
	...
	useEffect(() => {
		const timer = window.setTimeout(() => {
			void refreshCover(session.astdPath, manifest.frames);
		}, COVER_REFRESH_DEBOUNCE_MS);
		return () => window.clearTimeout(timer);
	}, [rasterOf, manifest.frames, session.astdPath]);

	/**
	 * 备注锚定的元素查询：位图态的 frame 先经 runLive 拉回活体，再逐条问引擎 bridge。
	 * 放置时的 hit-test 与 astd_notes 的锚点保鲜共用这一条。
	 */
	...
	useEffect(() => {
		...
		const rescan = async (seedOnly: boolean): Promise<void> => {
			...
			for (const file of files) {
				...
				// 影响面判定与活动态浮层共用（astd/bundle-paths），两边不能有两套说法。
	...
	// astd_screenshot 走的是 CanvasTab 注册的 controller，够不到这里的 runLive，
	...
	const worldStyle = useMemo(
		() =>
			({
				...
				"--astd-lscale": Math.min(1 / viewport.zoom, 8),
	...
	return (
		<div
			...
			className="relative h-full w-full select-none overflow-hidden outline-none astd-canvas-bg"
			style={{ cursor }}
			tabIndex={-1}
			role="application"
		...
		>
			<div ref={worldRef} className="absolute left-0 top-0" style={worldStyle}>
				...
				{marquee ? (
					<div
						className="absolute border border-[var(--astd-selected)] astd-marquee-fill"
