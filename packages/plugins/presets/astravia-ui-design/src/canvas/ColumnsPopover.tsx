...
export function ColumnsPopover({ current, max, boundaryRef, onPick, onClose }: ColumnsPopoverProps) {
	...
	return (
		<div
			// 不复用 .astd-ask-popover：那套样式自带指向正下方中点的尖角，而这个浮层是
			...
			<div className="grid grid-cols-5 gap-1">
				...
				{custom ? (
					<input
						...
						onBlur={submit}
						className="size-7 select-text rounded-md border border-[var(--astd-selected)] bg-card text-center text-xs tabular-nums text-foreground outline-none"
					/>
				) : (
					<button
						type="button"
