...
interface SnapGuidesProps extends SnapDecoration {
	 ...
	 *
	 * 刻意不复用 world 层的 `--astd-lscale`：那个变量有 `min(…, 8)` 的上限（标题栏
...
function GuideLine({ guide, thickness }: { guide: SnapGuide; thickness: number }) {
	...
	return (
		<div
			className="absolute bg-[var(--astd-selected)]"
			style={
				vertical
					? {
...
function GapLabel({ gap, scale, thickness }: { gap: SnapGap; scale: number; thickness: number }) {
	...
	return (
		<>
			...
			>
				<span
					className="absolute whitespace-nowrap rounded-sm bg-[var(--astd-selected)] px-1 text-[10px] leading-4 text-white"
