 ...
 *
 * 都是薄封装：真正的活儿在既有模块里（scaffoldDesign / importPackagedAstd /
 * exportDesign / official.projects），这里只负责把「项目」这一层拼上去。
 */
import { exportDesign } from "../export/export-design";
import { importPackagedAstd, parsePackagedAstd } from "../export/import-design";
 ...
 *
 * 刻意**不**预铺 `.astd`：设计稿的结构（有几屏、多大尺寸、什么主题）由用户的第一句
...
export function projectNameFromShareFile(fileName: string): string {
	return toProjectName(fileName.replace(/\.(?:astdz|vetdz|astd|vetd)$/i, "").replace(/-share$/i, ""));
}
...
export async function importDesignPackage(fileName: string, bytes: Uint8Array): Promise<CreatedDesign> {
	...
	const astdPath = await importPackagedAstd(ctx, packaged, entry.path, fileName);
	return { cwd: entry.path, astdPath };
}
 ...
 */
export async function exportDesignByPath(astdPath: string): Promise<string | null> {
	const ctx = getPluginCtx();
	const session = new DesignSession(ctx, astdPath);
	try {
		await session.open();
