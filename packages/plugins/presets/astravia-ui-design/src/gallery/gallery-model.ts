 ...
 *
 * 一张卡 = 一个项目。收集条件是项目根目录下直接躺着 `x.astd/` 包——只看一层，不做
 * 递归列举：画廊要对**所有**项目扫一遍，递归一个大仓库的代价会让这个页面打不开，
 * 而 scaffoldDesign 与导入本来就只往项目根写（`${cwd}/${name}.astd`）。
 * 代价是用户手动挪进子目录的设计收不到，这是明确取舍。
 */
import type { PluginFsApi } from "@astravia-org/plugin-sdk";
 import { designNameOf, MANIFEST_FILE, toAstdPath } from "../astd/manifest-types";
import { migrateLegacyDesign } from "../astd/migrate";
...
export async function scanProjectDesigns(fs: PluginFsApi, cwd: string): Promise<GalleryDesign[]> {
	...
	for (const entry of entries) {
		...
		if (legacyPath) {
			...
			const target = toAstdPath(legacyPath);
			const migrated = await migrateLegacyDesign(fs, target).catch(() => false);
			if (!migrated) continue;
			designs.push(await describeDesign(fs, target, entry.modifiedAt));
			continue;
		}
		if (!entry.isDirectory || !entry.name.endsWith(".astd")) continue;
...
/**
 * 这个条目是不是一份等待迁移的 v1 遗留设计（0.1.0 双文件格式），是则给出旧路径（`x.vetd`）。
 *
 * 两种形态：
...
function legacyDesignPathOf(
	...
	if (entry.isDirectory && entry.name.endsWith(LEGACY_SIDECAR_SUFFIX)) {
		...
		// 同名 `.astd` 目录已经在了，说明迁移早就完成、这个旁挂目录是别的东西的残留，
		// 不要去动它（rename 会撞上已存在的目标）。base 是旧路径（`.vetd`），换算成
		// 新路径（`.astd`）再比——同名设计包是新格式目录。
		if (bundleNames.has(toAstdPath(base))) return null;
		return entry.path.slice(0, -".d".length);
	}
	return null;
}

async function describeDesign(fs: PluginFsApi, astdPath: string, fallbackModifiedAt: number): Promise<GalleryDesign> {
	// 取 design.json 的 mtime 而不是目录的：目录 mtime 只在增删文件时变，而拖画框、
	// 平移视口这些「画布上刚干过的事」写的正是 manifest。
	const manifestStat = await fs.stat(`${astdPath}/${MANIFEST_FILE}`).catch(() => null);
	return {
		astdPath,
		name: designNameOf(astdPath),
