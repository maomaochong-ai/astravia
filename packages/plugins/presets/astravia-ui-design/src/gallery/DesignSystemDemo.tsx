...
export function DesignSystemDemo({ system, active, className }: DesignSystemDemoProps) {
	...
	useEffect(() => {
		...
		// 进视口才挂载 iframe，滚出视口（含下方余量）就卸载：首屏几十张卡同时
		// 解析三十多 KB 的 demo 文档正是画廊卡顿的根源，常驻数量必须压在
		// 视口行数附近，而不是越滚越多。
	...
	const animationName = `astd-demo-scroll-${system.id.replace(/[^a-z0-9-]/g, "")}`;
	...
	return (
		...
		// content-visibility：视口外的 demo 文档直接跳过渲染（布局/绘制），
		// 滚到才画；auto 模式用上次记住的尺寸稳住滚动条，避免跳动。
		<div
			ref={boxRef}
			data-active={active ? "" : undefined}
			className={`relative w-full overflow-hidden [content-visibility:auto] [contain-intrinsic-size:auto_220px] ${className ?? ""}`}
		>
