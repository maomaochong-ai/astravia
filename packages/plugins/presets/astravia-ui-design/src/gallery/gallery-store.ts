...
import { manifestPathOf, type AstdManifest } from "../astd/manifest-types";
import {
	type GalleryProject,
	hasRunningSession,
	scanProjectDesigns,
...
async function readAccent(astdPath: string): Promise<string | null> {
	try {
		const file = await getPluginCtx().fs.readFile(`${astdPath}/theme.css`);
 ...
 */
async function resolveCover(astdPath: string): Promise<string | null> {
	const cached = await loadCover(astdPath);
	if (cached) return cached;
	try {
		const raw = await getPluginCtx().fs.readFile(manifestPathOf(astdPath));
		const manifest = JSON.parse(raw.content) as AstdManifest;
		if (!Array.isArray(manifest.frames) || manifest.frames.length === 0) return null;
		const composed = await composeCover(astdPath, manifest.frames);
		if (composed) await saveCover(astdPath, composed);
		return composed;
	} catch {
...
export async function loadGallery(): Promise<GallerySnapshot> {
	...
	const [snapshot, runningCwds] = await Promise.all([
		...
		ctx.official.projects.listRuntimeProjects().then((value) => value as string[]).catch(() => [] as string[]),
	]);
	const cards = await Promise.all(
		snapshot.projects.map(async (project) => {
			...
			const [coverDataUrl, accent] = await Promise.all([
				resolveCover(card.cover.astdPath),
				readAccent(card.cover.astdPath),
			]);
			return {
				...card,
				coverDataUrl,
