 ...
 * 显式摊开给用户选，而不是替他挑一份。
 */
export function CardContextMenu({ anchor, onExport, onArchive, onClose }: CardContextMenuProps) {
		...
		>
			{designs.length === 1 ? (
				...
				<>
					...
					{designs.map((design) => (
						<MenuItem key={design.astdPath} label={design.name} onClick={() => onExport(design)} />
					))}
				</>
			)}
