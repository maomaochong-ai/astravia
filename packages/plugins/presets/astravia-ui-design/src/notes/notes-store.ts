import type { Disposable, PluginFsApi } from "@astravia-org/plugin-sdk";
import {
	type DesignNote,
	demoteAnchor,
	type NoteAnchor,
...
/**
 * 一份设计的备注。单写者：画布 UI 和 astd_notes 工具都通过这里做定点 patch，
...
export class NotesStore {
	...
	async load(): Promise<void> {
		 ...
		 * 落下或重开的备注生效；要清存量，面板上的「让 Astravia 处理」还在。
		 *
	...
	/**
	 * 放置时的后台 hit-test 落地，或 astd_notes 读取时的锚点保鲜：把 frame 锚升级成
	...
	private persist(): void {
		...
		this.writing = this.writing
			...
			.catch((error: unknown) => {
				console.error("astd notes write failed", error);
			});
	}
