...
/**
 * 「让 Astravia 处理」的会话闸口与发送动作。抽屉与气泡 thread 弹层共用，保证两个
 * 入口的可用性判断和提示词完全一致。
 *
 * 备注内容不进 prompt：agent 收到指令后自己调 astd_notes 拉全量（含现截的编号
 * 标注图），prompt 只负责把它指过去。
 */
/**
 * 发不出去的几种原因。要分辨它们，是因为「agent 正在跑」与其余几种有本质区别：
 * 它此刻就在射程内，收尾自检时会自己把备注捞走；其余几种则根本没有人会来取。
 *
 * `no-conversation` 指的是连该在哪个 workspace 开会话都不知道（画布没有 cwd）。
 * `no-active-session` 是宿主此刻没有活跃会话（用户停在新会话页）：SDK 不提供从
 * 插件创建会话的能力，得先请用户开一个会话。
 */
export type NotesHandoffBlock = "no-conversation" | "no-active-session" | "other-workspace" | "streaming";
...
export function useNotesHandoff(cwd: string | null): NotesHandoff {
	...
	/**
	 * 没有活跃会话（用户停在新会话页）时发不出去：SDK 不提供从插件创建会话的
	 * 能力（旧版 createSession 已随 SDK 重构移除）。画布知道 cwd 也没用——
	 * 先请用户开一个会话再派活。真正没辙的还有连 cwd 都没有的情况。
	 */
	...
	const BLOCK_MESSAGES: Record<NotesHandoffBlock, string> = {
		...
		"no-active-session": t("notes.handoff.noActiveSession"),
	...
	const send = (prompt: string): void => {
		...
		// 无活跃会话时 UI 已禁用按钮；这里兜底拦一下，不静默丢弃。
		if (sendNeedsSession) {
			notify({ message: t("notes.handoff.noActiveSession") });
			return;
		}
...
function dispatchAllPrompt(): string {
	return [
		...
		"Call astd_notes with no arguments to list every pending note — each comes with its position, a freshly re-resolved source anchor, and an annotated screenshot.",
		"Work through them ONE AT A TIME: make the change, verify it with astd_screenshot, then resolve that single note before starting the next.",
...
function dispatchOnePrompt(noteId: string): string {
	return [
		...
		"call astd_notes with that id to see its position, anchor and annotated screenshot, make the change, verify it with astd_screenshot, then resolve it.",
 ...
 * 自检时（SKILL.md 要求报告前无条件跑一次 astd_notes）会自己把这些捞走。
 *
...
export function useNotesAutoDispatch(notes: NotesStore, cwd: string | null): void {
	...
	useEffect(() => {
		...
		// 会话在别的 workspace、没有活跃会话、或画布连 cwd 都没有：没有人会来自检，
		// 所以留着不动，等闸口放行的那一刻再派出去。
