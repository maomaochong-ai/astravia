 ...
 *    全量重设（调用方负责发指令）。
 */
export async function applyDesignSystem(fs: PluginFsApi, astdPath: string, systemId: string): Promise<ApplyResult> {
	const system = designSystemById(systemId);
	if (!system) throw new Error(`unknown design system "${systemId}"`);
	const dirPath = astdPath;
	const manifest = JSON.parse((await fs.readFile(manifestPathOf(astdPath))).content) as AstdManifest;
	...
	if (frames.length === 0) {
		...
		return { mode: "direct", frames, astdPath, dirPath };
	}
	return { mode: "restyle", frames, astdPath, dirPath };
}
 ...
 * 语言跟宿主 locale（与 ask-astravia 同一取舍：协议串不进 locales catalog）。
 */
export function buildRestylePrompt(system: DesignSystem, result: ApplyResult, locale: string): string {
	...
	if (locale.toLowerCase().startsWith("zh")) {
		return [
			...
			"3. 每改完一个 frame 用 astd_screenshot 截图核对，全部完成后逐帧确认无溢出、无对不齐、无对比度问题。",
			"",
	...
	return [
		...
		"3. After each frame, verify with astd_screenshot; before reporting back, re-check every touched frame for overflow, misalignment and contrast issues.",
		"",
