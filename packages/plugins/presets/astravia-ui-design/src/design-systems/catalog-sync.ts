 ...
 * 目标）仍是防御性的——gitee 的 `/raw/` 会 302 到 `raw.giteeusercontent.com`，两个
 * host 都已声明，未来宿主若收紧白名单不会误伤。
 */
...
/** 清单在资源仓库中的固定位置。 */
const CATALOG_PATH_IN_REPO = ".astravia/design-templates.json";

/**
 * 由清单地址推出仓库根地址。
 *
 * 清单里的资源地址是**相对仓库根**的（`templates/<slug>/...`），而 `new URL(rel, base)`
 * 是相对清单所在目录解析的——直接拿清单地址当 base 会多出一段 `.astravia/`。
 */
