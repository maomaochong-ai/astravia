...
"Ask Astravia" edits, exported renders) must re-`fetch` every image and inline it as a data
...
Every `astd_screenshot` parses the sources first. A frame whose own file (or
`_layout.tsx`, or anything in `components/`) does not parse returns the syntax
error with its line instead of an image; `astd_status` reports the same thing
...
`astd_status` also returns the engine's recent build output, which is where to
