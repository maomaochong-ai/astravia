...
description: "Build and edit design documents (.astd) on the Astravia design canvas — app screens, landing pages, slides, posters, infographics. Use when the user asks for a UI design, mockup, screen, deck, or poster, or attaches a design frame/element from the canvas. Frames are real React (TSX) routes, not pictures."
---

# Astravia UI Design

A design document is ONE directory — `login-app.astd/` — holding everything:
...
login-app.astd/
  ...
  package.json          ← this design's npm dependencies; astd_install writes it
...
`astd_status` lists their sizes and the design's `defaultFrameSize` — match them
...
`astd_screenshot` and `astd_status`, naming the file and line. A file that does
...
- Normal formatting, one element per line for nested markup. Everything on one
  line destroys element→source mapping, and the user's "Ask Astravia" edits then
...
A design is a real npm project: `astd_install` adds packages to its own
`package.json`, and they travel with the design. `astd_status` lists what is
...
This is the first decision, not an afterthought: `astd_create` requires it
...
**New document**: pick the product type → `astd_create` with it → start building.
...
- After `astd_create`, write the pack's `theme.css` content into the design's
...
**Existing document**: `astd_status` ONCE first. It returns the frame ids/sizes,
...
again afterwards — `astd_screenshot` returns `issues` per frame.
...
2. **Detail pass, one frame at a time.** Fill in real content for one frame,
   then `astd_screenshot` it, then move to the next. Each save hot-reloads, so
...
version" is `astd_history` then `astd_restore`, NOT you editing files until they
resemble the old version from memory. Restoring is safe in both directions: it
is saved as a new version too, so a wrong pick is one more `astd_restore` away
...
**Verify visually**: `astd_screenshot` every frame you created or changed, then
...
renders — the checker only speaks through `astd_screenshot` and `astd_status`,
...
onto empty canvas) — each is a request addressed to you. `astd_notes` lists the
...
`astd_screenshot`, then immediately `resolve` that one note before you start the
...
user sends an explicit "handle my notes" prompt, `astd_status` reports a
`pendingNotes` count, and `astd_screenshot` flags pending notes on the frame it
just shot.
...
screenshot — which is exactly why the Done check below runs `astd_notes`
...
text lives in the notes, and `astd_notes` is where you read it. You finish each
one by replying through `astd_notes`'s `resolve`, which is what puts your answer
...
in `.notes.json` and reaches you through `astd_notes` — there is no separate
...
your summary — is one more `astd_notes` call.** Run it even when nothing this
...
- **Screenshot before revising.** Once a frame is written, the next thing you do
  to it is `astd_screenshot`. Do not polish it first — you cannot see what is
