...
1. **遵循 skill `plugin-workbench`** 的完整流水线（澄清 → 读文档 → 实现 → 标准脚本构建 → 引导用户在面板「应用到 Astravia」→ 验证）。
