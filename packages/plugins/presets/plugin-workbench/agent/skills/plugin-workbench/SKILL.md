...
description: >
  ...
  apply zip to Astravia, edit plugin.json name/guidingWords, or debug plugin load/install.
...
3. 用户插件依赖：`@astravia-org/plugin-sdk` / `@astravia-org/plugin-vite` 用 **registry 已发布 semver**（scaffold 默认 sdk `^0.1.0` / vite `^0.0.3`，两者版本独立；若 install 失败问用户 registry/版本）。  
...
**再次强调**：实现前 `read` `{workbenchRoot}/agent/docs/plugin/` 下对应文件；该目录即 Astravia 插件开发手册（与仓库 `docs/plugin` 同源同步）。
