...
Astravia 的系统 preset 插件，用节点画布组织提示词、素材生成与处理流程，并通过可扩展编排工作区完成内容创作。
...
- 项目存储：写入 `<cwd>/.astravia/content-creation/project.json`，带 schemaVersion 与 revision；无工作目录时回退到插件私有存储。
