...
describe("PluginContentProjectRepository", () => {
	it("stores project documents inside the active Windows project", async () => {
		...
		expect(harness.directories).toEqual(["C:\\project\\.astravia\\content-creation"]);
		expect(harness.files.has("C:\\project\\.astravia\\content-creation\\project.json")).toBe(true);
