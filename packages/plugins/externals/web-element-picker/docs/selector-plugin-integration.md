...
| 模块 | 文件 | 说明 |
|---|---|---|
| manifest | `manifest.json` | MV3，v0.3.2，`_locales` en/zh_CN，`<all_urls>` content script（document_start），web_accessible_resources 放行 `kernel-inject.js` |
...
| 构建 | `scripts/build-extension.mjs` | 读内核 bundle → 拼接 inject-main → 打包 content/background/popup → 复制资源 → 打 zip（`extension/release/web-element-picker-extension-0.3.2.zip`） |
