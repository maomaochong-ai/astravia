...
/** Astravia Desktop image tools: new bitmap → generate_image; revise existing → edit_image. */
const Astravia_IMAGE_TOOL_RULES = [
  '【Astravia 图片工具约定 — 必须遵守】',
...
const AI_SLIDES_ANNOTATION_EDIT_PROMPT = [
  '@cowart-Astravia 按标注修改 AI Slides',
  '',
 ...
 * against the renderer origin (vite HTML), not the MF plugin (`Astravia-plugin://…`).
...
function resolveCowartBundledAssetUrl(url) {
  ...
  if (
    url.startsWith('data:') ||
    url.startsWith('blob:') ||
    url.startsWith('Astravia-plugin:') ||
    /^https?:\/\//i.test(url)
  ) {
    return url
  }
...
async function exportCowartSlides(editor, slidesShapeId, format) {
  if (!hasCowartWidgetBridge()) {
    throw new Error('导出 Slides 文件夹需要在 Astravia Cowart 小组件中使用。')
  }
...
function buildHtmlDraftAnnotationEditPrompt({ draftShape, exportResult, screenshotAsset }) {
  ...
  return [
    ...
    '- If this already-open Astravia task does not expose updateExistingDraft yet, omit that argument but still pass replaceDraftHolder: false, placement: "right", margin: 40, matchAnchor: true, displayWidth and displayHeight. Never edit the original HTML file or original shape record.'
  ].join('\n')
}
...
async function sendAiImageGenerationRequest({ holderShape, userPrompt, referenceFiles = [] }) {
  ...
  for (const [index, referenceFile] of imageReferences.entries()) {
    ...
    if (hasCowartWidgetBridge()) {
      ...
      throw new Error('当前 Astravia host 没有声明支持图片附件，也没有可用的 Cowart MCP 文件保存桥。')
    }
