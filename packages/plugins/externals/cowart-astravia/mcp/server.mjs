...
function loadPluginManifest() {
  ...
  return { name: "cowart-astravia", version: "0.1.0" };
}
const pluginManifest = loadPluginManifest();
const COWART_ASTRAVIA_MODE =
  process.env.COWART_ASTRAVIA === "1" ||
  process.env.COWART_DISABLE_WIDGET === "1" ||
  pluginManifest.id === "cowart-astravia";
...
if (COWART_ASTRAVIA_MODE) {
  registerCowartAstraviaOpenTool(server);
} else {
...
function registerCowartAstraviaOpenTool(mcpServer) {
  mcpServer.registerTool(
    TOOL_RENDER_WIDGET,
    {
      title: "Open Cowart canvas (Astravia)",
      description:
        "Open/use the Cowart canvas for the active user project in Astravia Desktop. Pass projectDir as the user workspace (not the plugin install directory). Returns canvas storage paths; the host plugin should open the Cowart activity tab. Use other Cowart MCP tools for state and image IO.",
      inputSchema: {
        projectDir: z.string().trim().optional(),
        canvasDir: z.string().trim().optional(),
        ...
        content: [
          {
            type: "text",
            text: [
              "Cowart is running in Astravia mode (no Codex MCP App widget host).",
              "Open or focus the Cowart activity tab in Astravia Desktop for the canvas UI.",
