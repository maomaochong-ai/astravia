/**
 * Maps Codex MCP App widget bridge → Astravia plugin host APIs.
...
async function saveSnapshot(fs: PluginFsApi, projectDir: string, snapshot: unknown) {
	...
	// Full multi-page split is complex (asset localization). For Astravia 1:1 UX we still
...
export function installCowartAstraviaBridge(options: CowartBridgeOptions): () => void {
	...
	const callServerTool = async (request: { name: string; arguments?: Record<string, unknown> }) => {
		...
		try {
			...
			if (name === "insert_cowart_html_draft" || name === "insert_cowart_image") {
				...
				return {
					isError: true,
					content: [{ type: "text", text: `${name} should be invoked via agent MCP tools in Astravia.` }],
				};
			}
	...
	const cowartMcp = {
		...
		/**
		 * Dispatch a follow-up into the active Astravia conversation.
		...
		sendFollowUpMessage: async (message: unknown) => {
			...
			if (outcome === "started") {
				void sendPromise.catch((err: unknown) => {
					console.error("[cowart-astravia] conversation.sendPrompt failed after dispatch:", err);
				});
			}
			return {};
		},
	...
	(window as unknown as { openai: Record<string, unknown> }).openai = {
		...
		toolOutput: { projectDir, canvasDir, mode: "astravia" },
	...
	return () => {
		...
		if (win.openai && (win.openai as { toolOutput?: { mode?: string } }).toolOutput?.mode === "astravia") {
...
export function installBridgeFromPluginContext(ctx: PluginContext, projectDir: string): () => void {
	...
	if (!bridgeHold || bridgeHold.key !== key) {
		...
		bridgeHold = {
			key,
			dispose: installCowartAstraviaBridge({
				projectDir,
				fs: ctx.fs,
