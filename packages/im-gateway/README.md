...
- **All IM sessions live in `~/.astravia/conversation`** — the same default "对话" project desktop-app uses, so a conversation started in IM shows up in the desktop sidebar (with an "IM" badge) and vice-versa. No `/projects` / `/use` switching.
