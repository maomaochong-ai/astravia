...
const MACOS_ENV_WHITELIST = ["PATH", "LANG", "LC_ALL", "TERM", "ASTRAVIA_CLI_APP_PATH"] as const;
...
function buildMacosSandboxProfile(cwd: string, tempRoot: string, grant: SandboxShellGrant | undefined): string {
	...
	const sensitiveDenyPaths = [
		...
		join(homeDir, getAstraviaConfigDirName()),
		join(homeDir, ".pi"),
...
async function createAstraviaCliShim(
	...
	await writeFile(
		join(shimDir, "astravia"),
		["#!/usr/bin/env sh", `exec "${astraviaCliAppPath}" "$@"`, ""].join("\n"),
		{
			encoding: "utf8",
			mode: 0o755,
		},
		...
		exec: (command, cwd, { onData, signal, timeout, env }) => {
			return new Promise<{ exitCode: number | null }>((resolve, reject) => {
				void (async () => {
					...
					const tempRoot = await mkdtemp(join(tmpdir(), "astravia-macos-sandbox-"));
					...
					const child = spawn(sandboxExecPath, args, {
						cwd,
						detached: true,
						env: buildSandboxEnv(cwd, tempRoot, env, astraviaShimDir),
