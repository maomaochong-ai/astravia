...
import type { Agent, AgentEvent, AgentTool, ThinkingLevel } from "@astravia/agent-core";
import type { ImageContent, Model } from "@astravia/ai";
import type { HookConfigLayer } from "@astravia/ecosystem-adapter/hooks";
...
export interface AgentSessionConfig {
	 ...
	 * Default (via createAgentSession): Astravia-nested official layouts only —
	 * `~/.astravia/.codex/hooks.json`, `~/.astravia/.claude/settings.json`,
	 * `<cwd>/.astravia/.codex/hooks.json`, `<cwd>/.astravia/.claude/settings.json|settings.local.json`.
