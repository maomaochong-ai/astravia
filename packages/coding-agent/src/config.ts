import { DEFAULT_CONFIG_DIR_NAME, getAstraviaHomePath } from "@astravia/action-rpc";
...
export function getPackageDir(): string {
	...
	const envDir = process.env.ASTRAVIA_PACKAGE_DIR || process.env.PI_PACKAGE_DIR;
	if (envDir) {
...
export const PACKAGE_NAME: string = pkg.name || "@astravia/coding-agent";
export const APP_NAME: string = pkg.piConfig?.name || "astravia";
// 项目内配置目录名（cwd/<name>），用品牌默认名，不随 ASTRAVIA_CONFIG_DIR 变化。
// home 根目录名才受 ASTRAVIA_CONFIG_DIR 覆盖（见 action-rpc 的 getAstraviaHomePath）。
...
export function getAgentDir(): string {
	...
	return join(getAstraviaHomePath(), "agent");
}
...
/** Get the user-level skills directory (e.g., ~/.astravia/skills/) */
export function getUserSkillsDir(): string {
	return join(getAstraviaHomePath(), "skills");
}

/** Get the knowledge base root directory (e.g., ~/.astravia/knowledges/) */
