...
const vendorCacheDir = join(tmpdir(), "astravia-desktop-vendor-cache");
...
function resolvePlatformTagsFromEnv() {
	const rawTargets =
		process.env.ASTRAVIA_IM_GATEWAY_TARGET_PLATFORMS ??
		process.env.ASTRAVIA_CLI_TARGET_PLATFORMS ??
		process.env.ASTRAVIA_VENDOR_PLATFORM;
...
function resolveMacSigning() {
	...
	// ASTRAVIA_SKIP_NOTARIZE=1：保留签名、只跳过公证，供本地更新闭环快速迭代。
	...
	// 兜底：verify-mac-update.mjs 在 ASTRAVIA_REQUIRE_MAC_SIGNATURE=1 时会跑
	// `xcrun stapler validate`，未公证的产物过不了 test/stable 的发布门禁。
	const skipNotarize = process.env.ASTRAVIA_SKIP_NOTARIZE === "1";
...
// The CLI flow `Astravia --ocr-pdf <pdf> --output <json>` loads this HTML and
...
// ASTRAVIA_PACKAGE_DIR for assets; Windows additionally runs dist/cli.js via
...
// =============================================================================
//
// The agent-facing `astravia` command is @astravia/cli-app, not the desktop
...
async function stageVendorRuntimes() {
	...
	for (const type of ["node", "python"]) {
		...
		if (!entry) {
			throw new Error(
				`[prepare-pack] manifest 缺少 ${type} 平台 ${platformTag};跨平台打包请设 ASTRAVIA_VENDOR_PLATFORM`,
			);
		}
		...
		if (!readyArchive) {
			throw new Error(`[prepare-pack] 无法下载 vendor ${type}(${platformTag});检查构建机网络或设 ASTRAVIA_SKIP_VENDOR=1`);
		}
