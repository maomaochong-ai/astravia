...
- 跨领域共享的代码放入 `shared/`
- 可主题化的纯 UI 展现组件放入 `@astravia/theme-ui`；desktop-app 领域层只保留数据加载、状态、i18n 与事件适配，通过 props/view model 驱动 UI。
...
- 渲染进程通过 `window.astravia.*` 调用主进程功能
...
kebab-case namespace。默认根目录为 `~/.astravia/cache/`，例如 Marketplace 使用
`~/.astravia/cache/marketplace/`；不要再直接拼接 `*-cache` 或 `open-marketplace` 目录。
...
纯 `schemaVersion` 转换使用 `@astravia/toolkit/versioned-config`，文件读写可使用
`@astravia/toolkit/config-store`。业务 schema 与连续 migration 留在 desktop-app 对应领域内，
不要在业务模块重复实现迁移框架。
...
不要再把当天日志写到 `main.log`。旧版本遗留的 `~/.astravia/desktop-app/logs/main.log` 会在启动时迁移到 `main/legacy.*.migration.log`。
...
`desktop-app` 的主进程（`src/main/`）依赖 workspace 中的其他包（`@astravia/ai`、`@astravia/agent-core`、`@astravia/coding-agent` 等）。这些包的 `dist/` 目录必须先构建，否则 `build:main` 会报 "Failed to resolve entry for package" 错误。

**首次运行或依赖变更后**，必须先执行以下构建命令：

```bash
# 按依赖顺序构建 workspace 包
cd packages/ai && bun run build              # @astravia/ai
cd packages/agent && bun run build            # @astravia/agent-core
cd packages/tui && bun run build             # @mariozechner/pi-tui
cd packages/coding-agent && bun run build    # @astravia/coding-agent
...
- 消费 `@astravia/runtime-core` 的事件契约，契约变更需同步适配
