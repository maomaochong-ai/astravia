import type { BackgroundWorkViewItem } from "@astravia/theme-ui/activity";
import {
	activeSessionAtom,
	type BackgroundTask,
...
export function useBackgroundTasksTabPanelModel(): BackgroundTasksTabPanelModel {
	...
	const handleClearFinished = useCallback(() => {
		...
		void window.astravia.session.clearFinishedBackgroundTasks(sessionId);
	}, [sessionId]);
	...
	const handleStop = useCallback(
		(id: string, kind: "bash" | "subagent") => {
			if (!sessionId) return;
			if (kind === "bash") {
				void window.astravia.session.killBackgroundTask(sessionId, id);
			} else {
				void window.astravia.session.interruptSubagent?.(sessionId, id);
			}
		},
		[sessionId],
	);
