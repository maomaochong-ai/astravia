...
describe("resolveActivityTabs", () => {
	...
	const database = def({ id: "database", source: "builtin", order: 5, removable: false });
	const plugin = def({
		id: "plugin:git:changes",
		source: "plugin",
		pluginId: "git",
	...
	it("database builtin sorts by order and is restorable when hidden", () => {
		const result = resolveActivityTabs({
			definitions: [file, database, todo],
			metaById: metaMap({
				file: { label: "Files" },
		...
		// natural by order: file(0), database(5), todo(20)
		expect(result.onBar.map((t) => t.id)).toEqual(["file", "database", "todo"]);
		expect(result.onBar.some((t) => t.id === "database")).toBe(true);

		const hidden = resolveActivityTabs({
			definitions: [file, database, todo],
			metaById: metaMap({
				file: { label: "Files" },
				database: { label: "Database" },
		...
		expect(hidden.onBar.map((t) => t.id)).toEqual(["file", "todo"]);
		expect(hidden.restorable.map((t) => t.id)).toEqual(["database"]);
	});
});
