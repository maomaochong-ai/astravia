...
export type BuiltinMcpLabelKey =
	...
	| "mcpPresets.dbx.displayName"
	| "mcpPresets.dbx.description";
...
export const BUILTIN_MCP_PRESETS: readonly BuiltinMcpPreset[] = [
	...
	},
	{
		...
		// command 占位符 {{dbxMcpBin}} 在 main 侧写入 mcp.json 时展开为绝对路径（见 dbx-mcp-path.ts）。
		// 连接配置先走 dbx 连接存储（B1 手动配置）；B2 起由「数据库」面板接管。
		listedInDiscover: true,
		config: {
			command: "{{dbxMcpBin}}",
			args: [],
