...
export interface SettingsAiAssistOptions {
	/** 提交时附加到 agent instruction 的异步上下文（如数据库当前连接/表的 schema 摘要），失败时忽略。 */
	readonly buildExtraInstruction?: () => Promise<string>;
}

export function useSettingsAiAssist(
	tabId: SettingsAiAssistTabId,
	options?: SettingsAiAssistOptions,
): SettingsAiAssistModel | null {
	const buildExtraInstruction = options?.buildExtraInstruction;
	...
	const submit = useCallback(
		async (originFromClick?: DOMRect | null) => {
			...
			let finalInstruction = agentInstruction;
			if (buildExtraInstruction) {
				try {
					const extra = await buildExtraInstruction();
					if (extra) finalInstruction = `${agentInstruction}\n\n${extra}`;
				} catch {
					// 上下文获取失败不阻塞提交。
				}
			}
			...
			enqueueSettingsAssistJob(async () => {
				...
				await send(displayText, {
					metadata: {
						settingsAssistInstruction: finalInstruction,
		...
		[buildExtraInstruction, defaultConversationCwd, entry, intent],
	);
