...
export function useAgentSettingsModel(): AgentSettingsModel {
	...
	useEffect(() => {
		void window.astravia.session.getMaxRecentImages().then((value) => setMaxRecentImages(clampImages(value)));
	...
	const commitMaxRecentImages = useCallback((value: number) => {
		...
		void window.astravia.session.setMaxRecentImages(nextValue);
