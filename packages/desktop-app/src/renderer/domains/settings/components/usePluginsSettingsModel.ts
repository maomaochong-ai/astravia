...
export function usePluginsSettingsModel(): PluginsSettingsModel {
	...
	useEffect(() => {
		let cancelled = false;
		void (async () => {
			const all = await window.astravia.plugins.list();
			const configurable = all.filter((plugin) => (plugin.settingsSchema?.length ?? 0) > 0);
			if (cancelled) return;
			setPlugins(configurable);
			const entries = await Promise.all(
				configurable.map(
					async (plugin) => [plugin.id, await window.astravia.plugins.getSettings(plugin.id)] as const,
				),
			);
			if (cancelled) return;
			setValues(Object.fromEntries(entries));
		})();
		const unsubscribe = window.astravia.plugins.onSettingsChanged(({ pluginId, values: next }) => {
	...
	const update = (pluginId: string, key: string, value: unknown): void => {
		...
		void window.astravia.plugins.setSettings(pluginId, { [key]: value });
