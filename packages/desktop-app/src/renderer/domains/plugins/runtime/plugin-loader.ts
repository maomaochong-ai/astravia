...
function createSettingsApi(
	...
	const unsub = window.astravia.plugins.onSettingsChanged((payload) => {
...
function createNetworkApi(plugin: InstalledPlugin, capabilitySessionId: string): PluginNetworkApi {
	return {
		request: (request) => {
			...
			return window.astravia.plugins.networkRequest(capabilitySessionId, request);
		},
	};
}
...
function createStorageApi(plugin: InstalledPlugin, capabilitySessionId: string): PluginStorageApi {
	...
	return {
		readJson: (key) => {
			requireRead();
			return window.astravia.plugins.storageReadJson(capabilitySessionId, key);
		},
		writeJson: (key, value) => {
			requireWrite();
			return window.astravia.plugins.storageWriteJson(capabilitySessionId, key, value);
		},
		list: (prefix) => {
			requireRead();
			return window.astravia.plugins.storageList(capabilitySessionId, prefix);
		},
		readFile: (path) => {
			requireRead();
			return window.astravia.plugins.storageReadFile(capabilitySessionId, path);
		},
		writeFile: (path, data) => {
			requireWrite();
			return window.astravia.plugins.storageWriteFile(capabilitySessionId, path, data);
		},
		putBlob: (input) => {
			requireWrite();
			return window.astravia.plugins.storagePutBlob(capabilitySessionId, input);
		},
		readBlob: (id) => {
			requireRead();
			return window.astravia.plugins.storageReadBlob(capabilitySessionId, id);
		},
		getBlobRef: (id) => {
			requireRead();
			return window.astravia.plugins.storageGetBlobRef(capabilitySessionId, id);
		},
	};
}
...
function createCommandApi(plugin: InstalledPlugin, disposers: Array<() => void>): PluginCommandApi {
	...
	return {
		run: (file, args, options) => {
			...
			return window.astravia.plugins.runCommand(plugin.id, allowed, args ?? [], options);
		},
		spawn: async (file, args, options): Promise<PluginCommandSpawnHandle> => {
			...
			const result = await window.astravia.plugins.spawnCommand(plugin.id, allowed, args ?? [], options);
			let stopped = false;
			const stop = async (): Promise<void> => {
				if (stopped) return;
				stopped = true;
				await window.astravia.plugins.stopCommandSpawn(plugin.id, result.spawnId);
			};
			...
			return {
				...
				port: result.port,
				stop,
				status: () => window.astravia.plugins.getCommandSpawnStatus(plugin.id, result.spawnId),
				onExit: (listener) => {
...
export async function loadPlugin(plugin: InstalledPlugin, onChanged: () => void): Promise<LoadedPlugin> {
	...
	try {
		...
		const initialSettings = plugin.settingsSchema?.length
			? await window.astravia.plugins.getSettings(plugin.id).catch(() => ({}))
			: {};
		...
		await window.astravia.plugins.commitAppActionActivation(plugin.id, activationId);
