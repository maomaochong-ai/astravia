...
/**
 * B2.9-W1 查询同步通道（对话 → 工作台）：
 * - tool.start 时缓存 dbx_execute_query 的入参（连接 + SQL）；
 * - tool.end 成功时把 { connection, sql, result } 经 databaseTabTargetAtom 一次性写入，
 *   并激活活动面板 database tab（若未开且未被用户隐藏），由 database-tab 消费回填工作台。
 */
const dbxPendingQueryArgs = new Map<string, DbxExecuteQueryArgs>();

function syncDbxQueryToWorkbench(
	connection: string,
	sql: string,
	toolResult: unknown,
	cwd: string | null,
	...
	const rawText = extractDbxResultText(toolResult);
	// B2.10-W3：失败也回填——写入错误文案与原始详情（结果网格留空），用户能在工作台看到失败原因。
	store.set(
		databaseTabTargetAtom,
		isError
			? {
					...
					sql,
					result: null,
					error: i18n.t("settings:databaseQueryFailed"),
					errorDetail: rawText || undefined,
				}
			: {
					connection,
					sql,
					result: parseDbxQueryResultText(rawText),
				},
	);
	// B2.9-W3 埋点：AI 对话 dbx 查询结果回填工作台（对话 → 界面联动）。
	if (!cwd) return;
	// ADR-0026：用户手动隐藏过 database tab 时不再自动上栏（target 仍写入，手动打开时消费）。
	const hidden = store.get(hiddenActivityTabsAtom).get(cwd) ?? [];
	if (hidden.includes("database" as ActivityTabKey)) return;
	// 已显示 database tab 时只回填 target，不重复上栏（已挂载工作台经 syncTarget 消费）。
	const alreadyOnDatabase =
		store.get(activityPanelOpenAtom) &&
		store.get(activityPanelTabByProjectAtom).get(cwd) === ("database" as ActivityTabKey);
	if (alreadyOnDatabase) return;
	store.set(activityPanelTabByProjectAtom, (prev) => {
...
export function useSessionManager(): SessionManagerResult {
	...
	const openSession = useCallback(
		async (
			...
			currentUnsubscribe?.();
			// B2.9-W1：清掉未收尾的 dbx 工具入参缓存（会话切换时 tool.end 可能不再到来）。
			dbxPendingQueryArgs.clear();
			...
			const scenario: ConversationScenario =
				isBatchSession || isBatchProject || projectType === "batch"
					...
					: isDefaultConversation
						? "conversation"
						: "project";
			const createResult = await window.astravia.session.create(
			...
			const historyPromise = window.astravia.session.getFullHistory(sessionId);
			...
			const resolvedSessionPath = sessionPath ?? (await window.astravia.session.getSessionPath(sessionId)) ?? "";
			...
			const unsubscribeFn = await window.astravia.session.subscribe(sessionId, (event) => {
							...
							if (sp && cwd && rid && projectType !== "batch" && !autoTitledSessionsRef.current.has(sp)) {
								...
								void (async () => {
									...
									try {
										...
										const name = await window.astravia.session.autoTitle(rid, userText, assistantText);
							...
							if (rid && projectType !== "batch") {
								...
								void (async () => {
									try {
										...
										const suggestions = await window.astravia.session.nextPromptSuggestions(
											rid,
											conversation,
										);
				...
				if (event.type === "tool.start") {
					flushDeltas();
					// B2.9-W1：缓存 dbx_execute_query 入参（连接 + SQL），供 tool.end 成功时回填工作台。
					if (event.toolName === DBX_EXECUTE_QUERY_TOOL) {
						const parsedArgs = parseDbxExecuteQueryArgs(event.args);
						if (parsedArgs) dbxPendingQueryArgs.set(event.toolCallId, parsedArgs);
					}
					setChatMessages((prev) =>
						handleToolStart(
							prev,
				...
				if (event.type === "tool.end") {
					flushDeltas();
					// B2.9-W1/B2.10-W3：dbx_execute_query 结束（成功或失败）→ 激活 database tab + 一次性回填
					// SQL 与结果/错误，让用户在界面看到执行结果或失败原因。
					const pendingArgs = dbxPendingQueryArgs.get(event.toolCallId);
					if (pendingArgs) {
						dbxPendingQueryArgs.delete(event.toolCallId);
						syncDbxQueryToWorkbench(
							pendingArgs.connectionName,
							pendingArgs.sql,
							event.result,
							activeSessionRef.current?.cwd ?? null,
							event.isError,
						);
					}
					setChatMessages((prev) =>
	...
	const sendMessage = useCallback(
		async (
			...
			const effectiveOptions = pendingAssist
				? {
						...options,
						settingsAssistTabId: pendingAssist.settingsAssistTabId ?? options?.settingsAssistTabId,
						databaseTable: pendingAssist.databaseTable ?? options?.databaseTable,
						metadata: { ...pendingAssist.metadata, ...options?.metadata },
					}
				: options;
			if (pendingAssist) {
			...
			if (images) {
				try {
					imagePaths = await window.astravia.dialog.persistImages(
						session.runtimeId,
			...
			if (!streaming) {
				...
				const settingsAssistTabId = effectiveOptions?.settingsAssistTabId?.trim();
				if (settingsAssistTabId) {
					userMsg.settingsAssistTabId = settingsAssistTabId;
				}
				const dbTarget = effectiveOptions?.databaseTable;
				const dbConnection = dbTarget?.connection.trim();
				const dbTable = dbTarget?.table.trim();
				if (dbConnection && dbTable) {
					userMsg.databaseTable = { connection: dbConnection, table: dbTable };
				}
			...
			if (effectiveOptions?.metadata && Object.keys(effectiveOptions.metadata).length > 0) {
				promptReq.metadata = { ...effectiveOptions.metadata };
			}
			// B2.7: database table target rides as metadata so input-pipeline can inject
			// it model-only alongside the settings-assist instruction.
			const dbTarget = effectiveOptions?.databaseTable;
			const dbConnection = dbTarget?.connection.trim();
			const dbTable = dbTarget?.table.trim();
			if (dbConnection && dbTable) {
			...
			try {
				...
				await window.astravia.session.prompt(session.runtimeId, promptReq);
			} catch (err) {
		...
		async (runtimeId: string, id: string) => {
			...
			if (store.get(isStreamingAtom)) {
				...
				await new Promise<void>((resolve) => {
					...
					void window.astravia.session.abort(runtimeId).catch((err) => {
			...
			try {
				await waitForPluginHostReady();
				await window.astravia.session.prompt(runtimeId, item.request);
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
