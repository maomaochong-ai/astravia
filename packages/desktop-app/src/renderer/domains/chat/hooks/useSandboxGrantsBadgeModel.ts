import type { SandboxGrantsBadgeViewLabels, SandboxGrantViewItem } from "@astravia/theme-ui/chat";
...
export function useSandboxGrantsBadgeModel(): SandboxGrantsBadgeModel | null {
	...
	const refresh = useCallback(async () => {
		...
		try {
			const result = await window.astravia.session.listSandboxGrants(sessionId);
			setGrants(result);
		} catch {
	...
	const handleRevoke = useCallback(
		async (grantId: string) => {
			if (!sessionId) return;
			await window.astravia.session.revokeSandboxGrant(sessionId, grantId);
			await refresh();
		},
		[sessionId, refresh],
	);
	...
	const handleRevokeAll = useCallback(async () => {
		if (!sessionId) return;
		await window.astravia.session.revokeAllSandboxGrants(sessionId);
		await refresh();
	}, [sessionId, refresh]);
