...
import { shouldShowDbxAccessGuide } from "../../database/lib/dbx-access-guide";
...
function getPreviewImageSrc(item: FilePreviewItem): string {
	// Prefer path → astravia-file so stale file:// urls never win after history reload.
...
export function useUserMessageModel({
	...
	const applyEditFill = useCallback(async () => {
		...
		if (!entryId && activeSession?.runtimeId) {
			const history = await window.astravia.session.getFullHistory(activeSession.runtimeId);
	...
	const handleSwitchBranch = useCallback(
		(direction: -1 | 1) => {
			...
			runWithInterruptConfirm("switch", async () => {
				const runtimeId = activeSession.runtimeId;
				await window.astravia.session.switchBranch(runtimeId, targetId);
	...
	const handleFork = useCallback(() => {
		...
		runWithInterruptConfirm("fork", async () => {
			...
			const { path } = await window.astravia.session.forkSession(runtimeId, message.entryId!);
	...
	const performDelete = useCallback(
		async (suppressForOneMinute: boolean): Promise<void> => {
			...
			await window.astravia.session.deleteMessage(runtimeId, message.entryId);
	...
	const databaseTarget = message.databaseTable;
	...
	const setDatabaseTabTarget = useSetAtom(databaseTabTargetAtom);
	// AI 访问开关（database.dbxToolEnabled）实时读取：消息带表目标时异步拉一次，避免启动时快照过期。
	const [dbxAccessOpen, setDbxAccessOpen] = useState<boolean | null>(null);
	useEffect(() => {
		if (!databaseTarget) {
			setDbxAccessOpen(null);
			return;
		}
		let disposed = false;
		void window.astravia.config.get().then((config) => {
			if (disposed) return;
			setDbxAccessOpen(config.database?.dbxToolEnabled === true);
		});
		return () => {
			disposed = true;
		};
	}, [databaseTarget]);
	// 带表但访问未开启 → 展示「去开启」引导条（B2.10-W2 收尾，避免 AI 报 "tool not found" 时用户无感知）。
	const showDbxAccessGuide = shouldShowDbxAccessGuide(databaseTarget, dbxAccessOpen === true);
		...
		<>
			...
			{databaseTarget ? (
				<button
					...
					title={t("messageList.userMessage.openInDatabase", {
						connection: databaseTarget.connection,
						table: databaseTarget.table,
					})}
					onClick={() => {
						// B2.7 调整：不再跳 /database，改为激活活动面板数据库标签页（数据工作台），
						// 目标连接/表经一次性 target atom 传递，消费后清除——与点击文件打开预览流程一致。
						const cwd = activeSession?.cwd;
						if (cwd) {
				...
				>
					...
					{t("messageList.userMessage.openInDatabase")}
				</button>
			) : null}
			{showDbxAccessGuide ? (
				<button
					type="button"
					className="inline-flex items-center gap-1 rounded-md bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-500 transition-colors hover:bg-amber-500/20"
					title={t("messageList.userMessage.dbxAccessGuideTitle")}
					onClick={() => void navigate({ to: "/settings/$tab", params: { tab: "database" } })}
				>
					<span className="icon-[mdi--shield-alert-outline] h-3 w-3" />
					{t("messageList.userMessage.dbxAccessGuideAction")}
				</button>
			) : null}
		</>
	);
