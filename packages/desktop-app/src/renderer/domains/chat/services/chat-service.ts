...
export function fullHistoryToChat(entries: HistoryEntry[]): ChatMessage[] {
	...
	/** Next user message carries a database table target (B2.7 interface ⇄ chat). */
	let pendingDatabaseTable: { connection: string; table: string } | undefined;
	...
	for (const entry of entries) {
		if (entry.type === "compaction") {
			...
			pendingDatabaseTable = undefined;
		...
		if (entry.type === "settings_assist_marker") {
			...
			const db = entry.databaseTable;
			pendingDatabaseTable =
				db && typeof db.connection === "string" && typeof db.table === "string"
					? { connection: db.connection, table: db.table }
					: undefined;
			continue;
		}
		...
		if (m.role === "user") {
			...
			if (pendingDatabaseTable) {
				userMsg.databaseTable = pendingDatabaseTable;
				pendingDatabaseTable = undefined;
			}
			pendingPromptRef = undefined;
			pendingAttachments = undefined;
