	...
	const applyPluginSetup = useCallback(
		(item: PluginAbility, next: { enabled: boolean; grantedPermissions: PluginPermission[] }) => {
			run(`${item.id}:setup`, async () => {
				...
				if (grant.length > 0) await window.astravia.plugins.grantPermissions(item.slug, grant);
				if (revoke.length > 0) await window.astravia.plugins.revokePermissions(item.slug, revoke);
				// 授权先于启用：反过来的话插件会以缺权限的状态先 activate 一次并抛错。
				if (next.enabled !== item.enabled) await window.astravia.plugins.setEnabled(item.slug, next.enabled);
				notifyPluginsChanged();
			});
		},
		[run],
