...
export function useRootLayoutModel(): RootLayoutModel {
	...
	const confirmationQueueRef = useRef<
		Parameters<Parameters<typeof window.astravia.session.onConfirmationRequest>[0]>[0][]
	>([]);
	const confirmationActiveRef = useRef(false);

	useEffect(() => {
		const showRequest = (
			request: Parameters<Parameters<typeof window.astravia.session.onConfirmationRequest>[0]>[0],
		) => {
			...
			setSandboxPermissionDrawer({
				...
				onConfirm: () => {
					void window.astravia.session.respondToConfirmation(request.requestId, true);
					setSandboxPermissionDrawer(null);
					showNext();
				},
				onCancel: () => {
					void window.astravia.session.respondToConfirmation(request.requestId, false);
					setSandboxPermissionDrawer(null);
					showNext();
				},
			});
		};
		return window.astravia.session.onConfirmationRequest((request) => {
	...
	useEffect(() => {
		...
		const unsubscribeRequest = window.astravia.session.onQuestionRequest((request) => {
			liveRequests.set(request.requestId, request);
			setPendingQuestions((prev) => ({ ...prev, [request.sessionId]: request }));
		});
		const unsubscribeResolved = window.astravia.session.onQuestionResolved(
	...
	useEffect(() => {
		...
		void window.astravia.scheduler.getScheduledSessionPaths().then((paths) => {
