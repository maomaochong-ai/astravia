...
// 初值来自主进程同步暴露的 window.astravia.i18n.initialState（preference + language）。
...
function readInitialLanguage(): string {
	const state = window.astravia?.i18n?.initialState;
	if (state && isSupportedLanguage(state.language)) return state.language;
	const fromMain = window.astravia?.i18n?.initialLanguage;
...
export function initI18n(): void {
	...
	if (!subscribed && window.astravia?.i18n) {
		subscribed = true;
		window.astravia.i18n.onLanguageChanged((state) => {
