...
export function useTheme() {
	...
	useEffect(() => {
		async function syncWithNative() {
			...
			if (mode === "auto") {
				try {
					const native = await window.astravia.theme.getNative();
					isDark = native.shouldUseDarkColors;
				} catch {
					isDark = window.matchMedia("(prefers-color-scheme: dark)").matches;
				}
				await window.astravia.theme.set("system").catch(() => {});
			} else {
				isDark = mode === "dark";
				await window.astravia.theme.set(mode).catch(() => {});
			}
	...
	useEffect(() => {
		const unsubscribe = window.astravia.theme.onNativeChanged((info) => {
	...
	const setMode = useCallback(
		async (newMode: ThemeMode, transitionOptions?: ThemeTransitionOptions, themeNameOverride?: string) => {
			...
			if (newMode === "auto") {
				...
				await window.astravia.theme.set(newMode).catch(() => {});
				r = newMode;
			}
	...
	useEffect(() => {
		return window.astravia.theme.onModeRequested(({ mode: requestedMode }) => {
	...
	useEffect(() => {
		return window.astravia.theme.onChangeRequested(async ({ mode: requestedMode, themeId, cursorStyle }) => {
	...
	useEffect(() => {
		return window.astravia.theme.onStateRequested(getThemeSnapshot);
	}, [getThemeSnapshot]);

	useEffect(() => {
		return window.astravia.theme.onHelpRequested(() => ({
