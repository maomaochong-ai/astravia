...
export interface ChatMessage {
	...
	/** 数据库表目标（B2.7 界面⇄对话联动）：携带 connection/table，气泡可回跳「在界面打开」。 */
...
export interface SendMessageOptions {
	...
	settingsAssistTabId?: string;
	/** 数据库表目标（B2.7）：气泡展示「在界面打开」回跳 /database 并加载该连接/表。 */
	databaseTable?: { connection: string; table: string };
}
...
/**
 * B2.9 W2 预填待发（数据库「让 AI 分析此表」重设计）：不再自动直发开场白，
 * 而是把自然问句预填进输入框供用户编辑；schema 指令与表目标暂存于此，
 * 等用户真正发送（非 overrideText 直发）时随本轮 prompt 一起带上后清空。
 */
...
export const pendingAssistSendAtom = atom<PendingAssistSend | null>(null);
