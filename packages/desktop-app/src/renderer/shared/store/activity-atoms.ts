...
export interface DatabaseTabTarget {
	...
	/** B2.9-W1：AI 对话中 dbx_execute_query 执行成功的 SQL（工作台查询面板回填）。 */
	readonly sql?: string;
	/** B2.9-W1：AI 执行的结构化结果（工作台结果网格回填，一次性展示）。 */
	readonly result?: DbQueryResult | null;
	/** B2.10-W3：AI 执行失败的展示文案（工作台结果区提示；失败时 result 为 null）。 */
	readonly error?: string;
	/** B2.10-W3：AI 执行失败的原始错误详情。 */
	readonly errorDetail?: string;
}

/**
 * 对话→界面（B2.7 调整）：激活活动面板数据库标签页时的一次性目标传递。
 * 写入方（对话徽标）set 后由 database-tab 一次性消费并清除，避免残留导致重复跳转。
 */
