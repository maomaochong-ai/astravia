...
import { disposeDbxMcpClient } from "./database/dbx-mcp-client.js";
...
// verify:ui 隔离实例与用户真实应用并行运行，跳过全局单实例锁（仅验证环境变量下生效）。
const isUiVerificationInstance = process.env.ASTRAVIA_UI_VERIFICATION === "1";
const gotSingleLock = isCliMode || isUiVerificationInstance ? true : app.requestSingleInstanceLock();
if (!gotSingleLock) {
	app.exit(0);
} else {
	...
	app.whenReady().then(async () => {
		...
		// 开发模式：每次启动清空 HTTP 缓存。插件资源走 astravia-plugin://，remoteEntry.js
...
setQuitCleanup(async () => {
	...
	// 退出前释放 dbx-mcp 子进程，避免孤儿进程残留。
	try {
		await disposeDbxMcpClient();
	} catch (err) {
		mainLog.warn("dbx-mcp client dispose failed", err);
	}
});
