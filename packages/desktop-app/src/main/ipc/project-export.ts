...
const MANIFEST_NAME = "_astravia-export.json";
const MANIFEST_FORMAT = "astravia-project-export";
...
function detectSupportedType(projectDir: string): SupportedProjectType | "unsupported" {
	const metaPath = join(projectDir, ".astravia", "meta.json");
 ...
 * locks (`*.lock`) under `.astravia/sessions/` — they would be stale on the target
...
//   1. `.astravia/task-states.json` — `sessionPath` of each batch task
//   2. `.astravia/sessions/*.jsonl` — `cwd` in the session header line, plus
...
function rewriteAbsolutePathsAfterImport(newRoot: string, oldRoot: string): void {
	if (!oldRoot) return;
	rewriteJsonFile(join(newRoot, ".astravia", "task-states.json"), oldRoot, newRoot);
	const sessionsDir = join(newRoot, ".astravia", "sessions");
...
async function handleExport(projectDir: string): Promise<ExportProjectResult> {
	...
	const saveResult = await dialog.showSaveDialog({
		title: "导出项目",
		defaultPath: `${projectName}.astravia.zip`,
		filters: [{ name: "Astravia Project Export", extensions: ["zip"] }],
	});
...
async function handleImport(): Promise<ImportProjectResult | null> {
	...
	// keeps pointing at e.g. `/Users/you/.../<project>/.astravia/sessions/foo.jsonl`
...
/** 读取项目 `.astravia/meta.json`（用于识别 batch 等项目类型）；不存在或解析失败返回 null。 */
