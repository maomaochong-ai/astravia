...
import {
	...
	loginHttpMcpServer,
	loginMcpDeviceFlow,
} from "@astravia/coding-agent/core/mcp/index.js";
import { resolveConfigValue } from "@astravia/coding-agent/core/resolve-config-value.js";
...
/**
 * B2.10-W2：把「AI 访问数据库」开关（database.dbxToolEnabled）同步到 mcp.json 的 dbx server
 * disabled 标志。dbx 工具注册点（coding-agent McpManager）按 mcp.json 的 disabled 跳过初始化；
 * 开关关闭 = dbx.disabled = true = AI 无法调用 dbx 工具（彻底阻断 AI 触库，感知/访问权限分离）。
 * 幂等：dbx 不在 mcp.json 时无事可做（本就无工具可用）。
 */
async function syncDbxToolAccessGate(database: { dbxToolEnabled?: boolean } | undefined): Promise<void> {
	const enabled = database?.dbxToolEnabled === true;
	const config = await readMcpConfig();
	const dbx = config.mcpServers.dbx;
	if (!dbx) return;
	if (Boolean(dbx.disabled) === !enabled) return;
	await writeMcpConfig({ mcpServers: { ...config.mcpServers, dbx: { ...dbx, disabled: !enabled } } });
}
...
export function registerFsIpc(): () => void {
	...
	ipcMain.handle(CHANNELS.CONFIG_SET, async (_event, config: unknown) => {
		...
		const next: DesktopConfig = {
			...
			database:
				patch.database !== undefined ? mergeDatabaseConfig(current.database, patch.database) : current.database,
		};
		...
		// B2.10-W2：数据库配置变化时同步 dbx 访问门禁（关闭即 mcp.json dbx.disabled=true）。
		if (patch.database !== undefined) {
			await syncDbxToolAccessGate(next.database).catch((error) => {
				console.warn("[dbx-access-gate] sync failed", error);
			});
		}
	...
	ipcMain.handle(CHANNELS.MCP_SET, async (_event, config: unknown) => {
		...
		// B2.10-W2：保持「AI 访问数据库」开关对 dbx 的总闸语义——即使经 MCP 页添加/启用 dbx，
		// 开关关闭时 dbx 仍保持 disabled，否则会绕过访问门禁（感知/访问权限分离）。
		try {
			const desktop = await readDesktopConfig();
			await syncDbxToolAccessGate(desktop.database);
		} catch (error) {
			console.warn("[dbx-access-gate] enforce failed", error);
		}
