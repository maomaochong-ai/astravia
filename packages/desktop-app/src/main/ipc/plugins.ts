...
export function registerPluginsIpc(pluginActionService: PluginActionService): () => void {
	...
	ipcMain.handle("astravia:plugins:list", () => listVisiblePlugins());
	// 能力市场（我的）需要完整清单：按工作模式过滤会让另一模式下已装的插件凭空消失，
	// 用户会误以为能力已丢失。此处不过滤，模式差异由详情页的工作场景标注说明。
	ipcMain.handle("astravia:plugins:list-all", () => listPlugins());
	ipcMain.handle("astravia:plugins:install-from-archive", async (_event, archiveBuffer: unknown, options: unknown) => {
	...
	ipcMain.handle("astravia:plugins:install-from-url", (_event, url: unknown, options: unknown) =>
		installFromUrl(url, options),
	);
	ipcMain.handle("astravia:plugins:install-from-path", (_event, path: unknown, options: unknown) =>
		installFromPath(path, options),
	);
	ipcMain.handle("astravia:plugins:register-mode-gate", (_event, id: unknown) => {
		registerPluginModeGate(asPluginId(id));
		refreshAgentPlugins();
	});
	ipcMain.handle("astravia:plugins:set-contribution-mode", (_event, id: unknown, active: unknown) => {
		setPluginContributionMode(asPluginId(id), active === true);
		refreshAgentPlugins();
	});
	ipcMain.handle("astravia:plugins:uninstall", (_event, id: unknown) =>
		uninstallInstalledPlugin(pluginActionService, id),
	);
	ipcMain.handle("astravia:plugins:set-enabled", (_event, id: unknown, enabled: unknown) =>
		setInstalledPluginEnabled(pluginActionService, id, enabled),
	);
	ipcMain.handle("astravia:plugins:grant-permissions", (_event, id: unknown, permissions: unknown) =>
		grantInstalledPluginPermissions(id, permissions),
	);
	ipcMain.handle("astravia:plugins:revoke-permissions", (_event, id: unknown, permissions: unknown) => {
		const pluginId = asPluginId(id);
		const previous = listPlugins().find((candidate) => candidate.id === pluginId)?.grantedPermissions ?? [];
	...
	ipcMain.handle("astravia:plugins:grant-commands", (_event, id: unknown, names: unknown) => {
		const pluginId = asPluginId(id);
	...
	ipcMain.handle("astravia:plugins:revoke-commands", (_event, id: unknown, names: unknown) => {
		const pluginId = asPluginId(id);
		const previous = listPlugins().find((candidate) => candidate.id === pluginId)?.grantedCommandNames ?? [];
	...
	ipcMain.handle("astravia:plugins:command-spawn-stop", (_event, pluginId: unknown, spawnId: unknown) =>
		stopPluginCommandSpawn(asPluginId(pluginId), asPluginId(spawnId)),
	);
	ipcMain.handle("astravia:plugins:command-spawn-status", (_event, pluginId: unknown, spawnId: unknown) =>
		getPluginCommandSpawnStatus(asPluginId(pluginId), asPluginId(spawnId)),
	);
	ipcMain.handle("astravia:plugins:reload", (_event, id: unknown) => reloadInstalledPlugin(id));
	...
	ipcMain.handle("astravia:plugins:dev-watch-start", (_event, id: unknown, projectDir: unknown) => {
		const pluginId = asPluginId(id);
	...
	ipcMain.handle("astravia:plugins:dev-watch-stop", (_event, id: unknown) => {
		stopPluginDevWatch(asPluginId(id));
		refreshAgentPlugins();
	});
	ipcMain.handle(
		"astravia:plugins:agent-contributions-begin-load",
	...
	ipcMain.handle("astravia:plugins:app-action-response", (_event, requestId: unknown, result: unknown) => {
		pluginActionService.respond(asPluginId(requestId), result);
	});
	ipcMain.handle("astravia:plugins:continuation-register", (_event, pluginId: unknown, registration: unknown) => {
		registerDynamicContinuationProvider(asPluginId(pluginId), asContinuationRegistration(registration));
		refreshAgentPlugins();
	});
	ipcMain.handle(
		"astravia:plugins:system-prompt-provider-register",
	...
	ipcMain.handle(
		"astravia:plugins:system-prompt-provider-unregister",
	...
	ipcMain.handle(
		"astravia:plugins:continuation-unregister",
		(_event, pluginId: unknown, providerId: unknown, activationId: unknown) => {
			unregisterDynamicContinuationProvider(
				asPluginId(pluginId),
				asPluginId(providerId),
	...
	ipcMain.handle("astravia:plugins:agent-tool-register", (_event, pluginId: unknown, registration: unknown) => {
		const normalizedPluginId = asPluginId(pluginId);
	...
	ipcMain.handle(
		"astravia:plugins:agent-tool-unregister",
	...
	ipcMain.handle("astravia:plugins:agent-contributions-clear", (_event, pluginId: unknown, activationId: unknown) => {
		const normalizedPluginId = asPluginId(pluginId);
		const normalizedActivationId = asOptionalStringId(activationId, "agent tool activation id");
	...
	ipcMain.handle("astravia:plugins:get-settings", (_event, id: unknown) => getPluginSettings(asPluginId(id)));
	ipcMain.handle("astravia:plugins:set-settings", (_event, id: unknown, values: unknown) => {
		const pluginId = asPluginId(id);
		...
		for (const contents of webContents.getAllWebContents()) {
			contents.send("astravia:plugins:settings-changed", { pluginId, values: effective });
		}
	});
	ipcMain.handle("astravia:plugins:network:request", (_event, sessionId: unknown, request: unknown) => {
		return capabilityAdapter.requestNetwork(asPluginId(sessionId), request);
	});
	ipcMain.handle("astravia:plugins:storage:read-json", (_event, sessionId: unknown, key: unknown) =>
		capabilityAdapter.readStorageJson(asPluginId(sessionId), asPluginId(key)),
	);
	ipcMain.handle("astravia:plugins:storage:write-json", (_event, sessionId: unknown, key: unknown, value: unknown) =>
		capabilityAdapter.writeStorageJson(asPluginId(sessionId), asPluginId(key), value),
	);
	ipcMain.handle("astravia:plugins:storage:list", (_event, sessionId: unknown, prefix: unknown) =>
		capabilityAdapter.listStorage(asPluginId(sessionId), prefix === undefined ? undefined : asPluginId(prefix)),
	);
	ipcMain.handle("astravia:plugins:storage:read-file", (_event, sessionId: unknown, path: unknown) =>
		capabilityAdapter.readStorageFile(asPluginId(sessionId), asPluginId(path)),
	);
	ipcMain.handle("astravia:plugins:storage:write-file", (_event, sessionId: unknown, path: unknown, data: unknown) => {
	...
	ipcMain.handle("astravia:plugins:storage:put-blob", (_event, sessionId: unknown, input: unknown) =>
		capabilityAdapter.putStorageBlob(asPluginId(sessionId), input),
	);
	ipcMain.handle("astravia:plugins:storage:read-blob", (_event, sessionId: unknown, blobId: unknown) =>
		capabilityAdapter.readStorageBlob(asPluginId(sessionId), asPluginId(blobId)),
	);
	ipcMain.handle("astravia:plugins:storage:get-blob-ref", (_event, sessionId: unknown, blobId: unknown) =>
		capabilityAdapter.getStorageBlobRef(asPluginId(sessionId), asPluginId(blobId)),
	);

	return () => {
		ipcMain.removeHandler("astravia:plugins:list");
		ipcMain.removeHandler("astravia:plugins:list-all");
		ipcMain.removeHandler("astravia:plugins:install-from-archive");
		ipcMain.removeHandler("astravia:plugins:install-from-url");
		ipcMain.removeHandler("astravia:plugins:install-from-path");
		ipcMain.removeHandler("astravia:plugins:register-mode-gate");
		ipcMain.removeHandler("astravia:plugins:set-contribution-mode");
		ipcMain.removeHandler("astravia:plugins:uninstall");
		ipcMain.removeHandler("astravia:plugins:set-enabled");
		ipcMain.removeHandler("astravia:plugins:grant-permissions");
		ipcMain.removeHandler("astravia:plugins:revoke-permissions");
		ipcMain.removeHandler("astravia:plugins:grant-commands");
		ipcMain.removeHandler("astravia:plugins:revoke-commands");
		ipcMain.removeHandler("astravia:plugins:command-run");
		ipcMain.removeHandler("astravia:plugins:command-spawn");
		ipcMain.removeHandler("astravia:plugins:command-spawn-stop");
		ipcMain.removeHandler("astravia:plugins:command-spawn-status");
		stopAllPluginSpawns();
		ipcMain.removeHandler("astravia:plugins:reload");
		ipcMain.removeHandler("astravia:plugins:dev-watch-start");
		ipcMain.removeHandler("astravia:plugins:dev-watch-stop");
		ipcMain.removeHandler("astravia:plugins:agent-contributions-begin-load");
		ipcMain.removeHandler("astravia:plugins:agent-tool-register");
		ipcMain.removeHandler("astravia:plugins:agent-tool-unregister");
		ipcMain.removeHandler("astravia:plugins:agent-contributions-clear");
		ipcMain.removeHandler("astravia:plugins:app-action-register");
		ipcMain.removeHandler("astravia:plugins:app-action-activation-commit");
		ipcMain.removeHandler("astravia:plugins:app-action-activation-abort");
		ipcMain.removeHandler("astravia:plugins:app-action-unregister");
		ipcMain.removeHandler("astravia:plugins:app-action-response");
		ipcMain.removeHandler("astravia:plugins:continuation-register");
		ipcMain.removeHandler("astravia:plugins:continuation-unregister");
		ipcMain.removeHandler("astravia:plugins:system-prompt-provider-register");
		ipcMain.removeHandler("astravia:plugins:system-prompt-provider-unregister");
		ipcMain.removeHandler("astravia:plugins:get-settings");
		ipcMain.removeHandler("astravia:plugins:set-settings");
		ipcMain.removeHandler("astravia:plugins:network:request");
		ipcMain.removeHandler("astravia:plugins:storage:read-json");
		ipcMain.removeHandler("astravia:plugins:storage:write-json");
		ipcMain.removeHandler("astravia:plugins:storage:list");
		ipcMain.removeHandler("astravia:plugins:storage:read-file");
		ipcMain.removeHandler("astravia:plugins:storage:write-file");
		ipcMain.removeHandler("astravia:plugins:storage:put-blob");
		ipcMain.removeHandler("astravia:plugins:storage:read-blob");
		ipcMain.removeHandler("astravia:plugins:storage:get-blob-ref");
