...
export function registerAbilitiesIpc(): () => void {
	...
	const unsubscribeFromUpdates = openMarketplace.subscribeToUpdates(() => {
		for (const win of BrowserWindow.getAllWindows()) {
			if (!win.isDestroyed()) win.webContents.send("astravia:abilities:open-marketplaces-updated");
		}
	});
	// 一次性下发全量台账；读取时顺带剔除漂移条目（ADR-0049）。
	ipcMain.handle("astravia:abilities:get-ledger", () => readAbilityLedger());
	ipcMain.handle("astravia:abilities:list-open-marketplace", async () => {
		const { source: _source, ...snapshot } = await openMarketplace.listSource(DEFAULT_MARKETPLACE_SOURCE_ID);
		return snapshot;
	});
	ipcMain.handle("astravia:abilities:refresh-open-marketplace", async () => {
		const { source: _source, ...snapshot } = await openMarketplace.refreshSource(DEFAULT_MARKETPLACE_SOURCE_ID);
		return snapshot;
	});
	ipcMain.handle("astravia:abilities:list-open-marketplaces", () => openMarketplace.list());
	ipcMain.handle("astravia:abilities:refresh-open-marketplaces", () => openMarketplace.refresh());
	ipcMain.handle("astravia:abilities:list-marketplace-sources", () => openMarketplace.listSources());
	ipcMain.handle("astravia:abilities:add-marketplace-source", (_event, input: unknown) =>
		openMarketplace.addSource(parseAddSourceInput(input)),
	);
	ipcMain.handle("astravia:abilities:update-marketplace-source", (_event, id: unknown, input: unknown) =>
		openMarketplace.updateSource(requireString(id, "id"), parseUpdateSourceInput(input)),
	);
	ipcMain.handle("astravia:abilities:remove-marketplace-source", (_event, id: unknown) =>
		openMarketplace.removeSource(requireString(id, "id")),
	);
	ipcMain.handle("astravia:abilities:refresh-marketplace-source", (_event, id: unknown) =>
		openMarketplace.refreshSource(requireString(id, "id")),
	);
	ipcMain.handle(
		"astravia:abilities:install-open-ability",
	...
	ipcMain.handle(
		"astravia:abilities:record-mcp-install",
	...
	return () => {
		...
		ipcMain.removeHandler("astravia:abilities:get-ledger");
		ipcMain.removeHandler("astravia:abilities:record-mcp-install");
		ipcMain.removeHandler("astravia:abilities:list-open-marketplace");
		ipcMain.removeHandler("astravia:abilities:refresh-open-marketplace");
		ipcMain.removeHandler("astravia:abilities:list-open-marketplaces");
		ipcMain.removeHandler("astravia:abilities:refresh-open-marketplaces");
		ipcMain.removeHandler("astravia:abilities:list-marketplace-sources");
		ipcMain.removeHandler("astravia:abilities:add-marketplace-source");
		ipcMain.removeHandler("astravia:abilities:update-marketplace-source");
		ipcMain.removeHandler("astravia:abilities:remove-marketplace-source");
		ipcMain.removeHandler("astravia:abilities:refresh-marketplace-source");
		ipcMain.removeHandler("astravia:abilities:install-open-ability");
	};
}
