...
const OFFICIAL_MARKETPLACE_REPOSITORY = "https://github.com/maomaochong-ai/astravia-official-marketplace";
...
function createDefaultSources(now: Date): MarketplaceSource[] {
	...
	const ref = validateRef(process.env.ASTRAVIA_OPEN_MARKETPLACE_REF);
	...
	return [
		{
			...
			archiveUrl:
				process.env.ASTRAVIA_OPEN_MARKETPLACE_ARCHIVE_URL ?? marketplaceArchiveUrl(normalizedRepository, ref),
			ref,
			enabled: true,
			builtin: true,
			autoUpdate: true,
