...
export function broadcastPluginsChanged(): void {
	for (const contents of webContents.getAllWebContents()) {
		...
		try {
			contents.send("astravia:plugins:changed");
		} catch {
			// ignore gone frames
		}
	}
 ...
 * 规避 Chromium 对 astravia-plugin:// 资源的缓存），Iconify/外链原样返回。
 */
...
function toPluginUrl(pluginId: string, version: string, relativePath: string): string {
	...
	return `astravia-plugin://${pluginId}/${versionedPath(version, normalized)}?v=${encodeURIComponent(version)}`;
}
 ...
 * 正式发布 / 关热更新后的持久授权仍走「应用到 Astravia / 重新安装」。
 */
...
const pluginSettingsPath = join(getAstraviaHomePath(), "plugin-settings.json");
...
export function reloadPlugin(id: string): InstalledPlugin {
	...
	// dev 链接期间：注册表照常应用 pendingVersion（否则「应用到 Astravia」的新版本会被
