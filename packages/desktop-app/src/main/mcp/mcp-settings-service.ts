...
import type { McpServerDetail, McpServerSummary, McpServerUpsertData } from "@astravia/capability-sdk";
...
import { expandDbxMcpCommand } from "./dbx-mcp-path.js";
...
export async function writeMcpConfig(config: McpConfigData): Promise<void> {
	atomicWriteJSON(MCP_CONFIG_PATH, validateMcpConfig(expandDbxMcpCommand(config)));
}
