...
describe("ElectronUpdaterEngine", () => {
	...
	it("marks the app as quitting before activating the Inno update", async () => {
		...
		const updateInfo: UpdateInfo = {
			version: "1.2.3",
			files: [{ url: "Astravia-1.2.3-win-x64.exe", sha512: "hash", size: 42 }],
		...
		const innoUpdate = {
			select: vi.fn().mockReturnValue({ assetFileName: "Astravia-1.2.3-win-x64.exe", totalBytes: 42 }),
	...
	it("waits for Squirrel.Mac to stage the update before reporting download completion", async () => {
		...
		const updater = {
			on: vi.fn(),
			off: vi.fn(),
			downloadUpdate: vi.fn().mockResolvedValue(["/tmp/Astravia.zip"]),
		} as unknown as AppUpdater;
		...
		await expect(download.promise).resolves.toEqual(["/tmp/Astravia.zip"]);
	...
	it("passes provider-resolved asset URLs to the Inno updater", async () => {
		...
		const prepareDownloadedInstaller = vi.fn().mockResolvedValue(["C:\\staged\\Astravia.exe"]);
		...
		const updater = {
			...
			downloadUpdate: vi.fn().mockResolvedValue(["C:\\pending\\Astravia-1.2.3-win-x64.exe"]),
		...
		await expect(engine.downloadUpdate(vi.fn()).promise).resolves.toEqual(["C:\\staged\\Astravia.exe"]);
		expect(prepareDownloadedInstaller).toHaveBeenCalledWith(
			"C:\\pending\\Astravia-1.2.3-win-x64.exe",
			expect.any(Function),
	...
	it("promotes the differential cache baseline before running Inno Setup", async () => {
		const root = await mkdtemp(join(tmpdir(), "astravia-updater-engine-"));
		temporaryRoots.push(root);
		const cacheDir = join(root, "cache");
		const installerPath = join(cacheDir, "pending", "Astravia-1.2.3-win-x64.exe");
