...
Stateful agent with tool execution and event streaming. Built on `@astravia/ai`.
...
```bash
npm install @astravia/agent-core
```

## Quick Start

```typescript
import { Agent } from "@astravia/agent-core";
...
```typescript
import { Agent, streamProxy } from "@astravia/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@astravia/agent-core";
