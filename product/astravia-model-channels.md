...
1. **发申请邮件**（模板见第 7 节）——这是启动流程的第一步。**注意：Remonker 官网没有白标自助入口**（实测 `remonker.io/whitelabel`、`/contact` 均返回 404，首页与 Pricing 导航无任何白标入口），白标必须通过联系商务/支持**人工申请**，渠道为 Dashboard 内的 Support（支持）或注册邮箱收到的客服回复地址；
...
来源：**https://remonker.io/pricing**（公开页，€ / 每百万 token，2026-03 更新）与后台 **https://remonker.io/dashboard/models**（credits 计价，**1 cr = €0.01**，与官网欧元价一一对应）。下表为 Astravia 计划开放的核心模型的 **Remonker 官网公开价**（欧元），混合成本按输入:输出 = 3:1 计算：`60M × (0.75 × 输入价 + 0.25 × 输出价)`。

| 模型 | 输入 €/M（Remonker） | 输出 €/M（Remonker） | 60M/月混合成本（€） | 备注 |
|---|---|---|---|---|
| GPT-5.4 | 0.91 | 7.28 | ~150 | 官网 -48% |
| Claude Sonnet 4.6 | 1.15 | 5.75 | ~138 | 官网 -62% |
| Claude Opus 4.6 | 1.95 | 9.75 | ~234 | 官网 -61% |
| Gemini 2.5 Pro | 1.13 | 4.50 | ~118 | 后台确认 113cr/450cr（-10%） |
| Gemini 3 Flash | 0.20 | 1.20 | ~27 | 官网 -60% |
| Grok 4.1 Fast | 0.24 | 0.59 | ~20 | 官网 -91% |
| Gemini 3 Pro | 1.76 | 7.02 | ~185 | 后台实测 176cr/702cr，市场价无折扣 |
| Gemini 3.1 Pro | 0.70 | 4.50 | ~99 | 后台实测 70cr/450cr（-65%/-63%） |
| DeepSeek V3 / Llama 3.3 | — | — | — | 待后台 Models 页查询 |

提醒：公开价页只列出 11 个主流模型，DeepSeek / Llama 等开源模型需登录后台 Models 页查看；正式商务报价（尤其白标批量折扣）以商务邮件为准。
