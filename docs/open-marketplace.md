...
Desktop 从 GitHub 下载完整仓库归档，并在本地读取 `.astravia/marketplace.json`。GitHub 不承担搜索、筛选或分页；这些操作全部基于客户端已校验的本地快照完成。
...
MCP 没有可执行安装产物，但必须有独立包目录。`marketplace.json` 只通过 `source.path` 指向目录，运行配置放在目录内的 `mcp.json`。客户端同步时读取并校验该文件，再在用户安装时把 `server` 写入 `~/.astravia/agent/mcp.json` 的 `mcpServers[slug]`。索引内联 `config.mcp` 会被拒绝，避免索引与包文件形成两个真相源。
...
- `ASTRAVIA_OPEN_MARKETPLACE_REPOSITORY`：GitHub 仓库 URL；未设置时不创建内置来源。
- `ASTRAVIA_OPEN_MARKETPLACE_REF`：分支或 ref，默认 `main`。
