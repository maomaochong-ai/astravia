...
rebrand/品牌落地蓝图.md 的 P5 要求 `build/icon.png` / `icon.ico` / `icon.icns` 承载 astra（星辰）+ via（路）的词源意象，蓝图第 50 行定为「星环 + 轨道」。当前 `build/icon.*` 是早期 AI 生成占位资源，应用后出现四周硬边、尺寸过大——根因是 AI 生成图标未预留 padding / safe area，且多尺寸（16/32/64/128/256/512/1024）一致性无法保证。代码层（appId / productName / 协议）已在 P2 就绪，但视觉资产若不定设计语言就产出，会反复返工，且后续品牌演进时缺乏判断「当前图标是否仍合规」的依据。需在产出前定下设计语言与产出方式，使本次产出成为可复用的品牌资产基线。
