...
把插件系统从 demo 级推向「对外开放 agent 对话常用 API」时，须先定信任模型，它决定每个 API 的形状。现状：插件已在 renderer 进程内执行，经 Module Federation 共享宿主 React/JSX/`@astravia-org/plugin-sdk` 单例（`plugin-host-shim`），`PluginContext` 实际只暴露 `ui.registerGlobalSlot` + `permissions`，声明的 `agent.session.*` / `fs.*` 等权限位是占位、无任何实现。
...
- `@astravia-org/plugin-sdk` 导出的 hook 要读宿主 atom，又不能反向依赖 desktop-app：靠宿主在 `installPluginHostShim` 时把 store/atoms/actions 注入 plugin-sdk 内部 bridge，依赖 Module Federation 的「宿主与插件共享同一份 pluginSdk 实例」成立。
