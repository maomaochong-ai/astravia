# Astravia UI Design：frame 走 vite 工程路线，而非纯浏览器运行时

「Astravia UI Design」系统插件的 frame 需要 Tailwind 与图标能力。我们决定 frame 的源码形态是 **TSX（React 组件）**，由插件托管的**共享引擎**（单一锁版本的 vite + React + Tailwind v4 + @iconify/tailwind4 模板工程，node_modules 预装）编译，每个打开的 .vetd 起一个 vite dev server，画布 iframe 指向 localhost 获得真 HMR（<100ms、不丢状态）——而不是「frame = 静态 HTML + `@tailwindcss/browser` 运行时」的零构建路线。
