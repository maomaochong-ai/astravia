...
> `~/.astravia/scene/`、对模型隐藏、触发时用 tasks.json 锁定 todo 列表」的 SOP 变体，
...
统一 **存放**（一张表）与 **呈现**（一个列表、一套详情页、一套图标口径、一套分类）；**不统一安装**——skill 装 `~/.astravia/skills/`、scene 装 `~/.astravia/scene/`、plugin 装 `~/.astravia/plugins/`、mcp 写进 `mcp.json` 的一个 key。三条轨道是三种不同的运行时机制，合并它们只会造出一层无收益的抽象。
...
- **命名沿用 Capability**：desktop 零重命名，但与 `@astravia/capability-runtime` 的授权契约层彻底同名，代码里无法靠名字分辨两个完全不同的东西。改为市场条目叫 Ability、授权层中文让出「能力」一词。
