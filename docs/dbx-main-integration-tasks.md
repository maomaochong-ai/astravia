...
- [ ] 反向（v2 可选）：工作台执行 SQL 后，结果网格提供「让 AI 解读此查询」入口（携带 SQL + 结果摘要跳转对话）。——**未实施**（v2 可选，随 V2–V6 排期）
...
### B2.10 反馈 4/5/7 定案：关键信息行圆角容器 + AI 感知/访问权限分离 + AI 协助功能定位（2026-08-16 新增反馈，1–2 人天）✅ W1–W3 已实施（2026-08-16，`bun run check` 全绿）；W4 排期 V2/B3.1
...
- [ ] **W4 感知级别细化排期（反馈 5，B2.10 外独立任务）**：连接级/表级选择性注入（`database.schemaInjection.scope: "all" | "connections[]" | "tables[]"`，排期 V2 系列或 B3）；生产/开发环境标记（连接字段 `env: prod|dev`，生产连接默认禁止写操作、敏感库需显式授权）排期 B3.1。
...
- [ ] 安全策略定制：执行模式默认最严、连接白名单管理、危险 SQL 拦截规则
- [ ] 执行限制：行数上限可配、超时控制
- [ ] 凭据安全加固（dbx 连接存储密码明文风险评估与整改）
