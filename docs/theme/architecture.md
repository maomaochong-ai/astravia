...
- SDK：`useThemeStorage` / `useThemeStorageValue`（`@astravia/theme-sdk/storage`）。
...
主题需要根据当前页面做视觉判断或受控导航时，应使用 `@astravia/theme-sdk` 暴露的 `useThemeRouteModel()`。它只返回稳定路由语义，例如 `pathname` 和 `area`，不暴露 TanStack Router 实例、route match、内部 params 或 search。
...
`@astravia/theme-ui` 是可选依赖，主题可以复用它，也可以完全自定义 UI。新增可复用组件的具体标准见 [组件设计要求](./component-guidelines.md)。

进入 `@astravia/theme-ui` 或官方默认 UI 包的组件应优先是 props-driven view。调用 SDK hook 的 connected container 留在 desktop-app，作为应用默认入口和 host adapter 的一部分。
...
```ts
import type { ThemeModule } from "@astravia/theme-sdk";
...
- React / ReactDOM 单例共享。
- `@astravia/theme-sdk` 单例共享。
- 可选 `@astravia/theme-ui` 单例共享。
- SDK 版本声明和能力声明。
- ErrorBoundary。
- 加载失败回退默认 UI。
- 权限声明。
