...
- `@astravia/theme-sdk/storage`：`useThemeStorage`、`useThemeStorageValue`。
- desktop-app host + preload IPC + `~/.astravia/desktop-app/themes/<themeId>/data.json`。
...
主题包未来不是通过 npm 安装进应用，而是在应用运行时动态加载。主题作者可以在开发期依赖 `@astravia/theme-sdk` 的类型和 `@astravia/theme-ui` 的可选组件；运行时由应用主题加载器提供这些 shared singleton。
