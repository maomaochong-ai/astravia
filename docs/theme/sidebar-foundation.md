...
- 主题公开的 `useSidebarModel` 入口来自 `@astravia/theme-sdk/sidebar`。
...
侧边栏 region 不负责直接访问内部 store、router 或 IPC。它可以使用 `SidebarRegionProps.model` 中的状态和 actions；如果主题自己组合完整 region，也可以调用 `@astravia/theme-sdk/sidebar` 暴露的 `useSidebarModel` facade，再把 model 作为 props 传给 props 驱动 view。
