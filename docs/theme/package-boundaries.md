...
`@astravia/theme-sdk` 只表达主题如何接入应用，不表达具体视觉。
...
```ts
import { useSidebarModel } from "@astravia/theme-sdk/sidebar";
```

这个 hook 从 `ThemeHostProvider` 读取 desktop-app 注入的实现。主题只能看到稳定 model 和 actions，不能接触内部 atom、router 或 `window.astravia`。

## Theme UI

`@astravia/theme-ui` 是可选 UI 库，不是主题协议的一部分。
...
主题可以复用 `@astravia/theme-ui`，也可以完全不用它。`@astravia/theme-ui` 不能成为所有主题实现的集合；具体主题组件应留在具体主题包里。
...
- 在运行时把 `react`、`@astravia/theme-sdk` 和可选 `@astravia/theme-ui` 作为 shared singleton 暴露给远程主题。

desktop-app 不应该要求主题 import 内部路径，例如 `@shared/*`、`@domains/*` 或 `window.astravia.*`。
...
3. 动态加载主题 bundle。
4. 将 `react`、`react-dom`、`@astravia/theme-sdk` 和可选 `@astravia/theme-ui` 映射为应用内置 singleton。
5. 渲染主题提供的 `ThemeModule`。
6. 加载失败时回退默认主题。

新增 SDK 子路径导出时，desktop-app runtime shared 配置和主题包 federation shared 配置必须同步登记对应子路径，例如 `@astravia/theme-sdk/routing`。否则主题包可能加载到另一份 SDK 实例，导致 `ThemeProvider` 或 `ThemeHostProvider` context 读取失败。主题组件优先从 `@astravia/theme-sdk` 根入口导入已 re-export 的 hook；只有需要子路径边界时才使用子路径导入。
...
后续如果要让默认 UI 组件被远程主题稳定复用，应把 props 驱动的 view 迁入独立默认主题 UI 包或更明确的 desktop theme UI 包，而不是放进 `@astravia/theme-sdk`。connected 容器继续留在 desktop-app。
