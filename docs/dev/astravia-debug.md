...
Astravia Debug 是开发环境专用的会话能力，用于从外层 Agent 驱动真实 Astravia Agent，完成创建会话、继续会话、处理 Ask User、等待结果和中止操作。

它与 Astravia Action 共用本地 RPC 服务，但能力目录相互独立。打包环境不注册 Debug runtime，调用时会返回 `DEBUG_NOT_AVAILABLE`。
...
验证实例使用当前工作树独立的 Astravia 配置、Electron user-data、RPC endpoint 和 CDP endpoint，不与普通开发实例共用运行状态。
