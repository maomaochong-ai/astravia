...
```powershell
cd C:\develop\yiyun\astravia-mono\packages\ecosystem-adapter
bun run build
...
```powershell
cd C:\develop\yiyun\astravia-mono\packages\desktop-app
bun dev
```
...
```powershell
cd C:\develop\yiyun\astravia-mono
...
```powershell
Select-String -Path "$env:USERPROFILE\.astravia\desktop-app\logs\main\*.log" `
...
```powershell
$env:ASTRAVIA_BASH = "C:\Program Files\Git\bin\bash.exe"
```
...
```text
<project>/.astravia/claude-hooks.json
```

写入与 fixture 相同结构的 hooks，重新开会话即可。不要写入 `.astravia/hooks.json`（那是 Codex profile）。

## E. Playwright（可选 UI）

```powershell
playwright-cli -s=astravia attach --cdp=http://127.0.0.1:9223
playwright-cli -s=astravia tab-list
# 选择 Title=Astravia Desktop 的 tab
playwright-cli -s=astravia tab-select <n>
playwright-cli -s=astravia snapshot
playwright-cli -s=astravia detach
```
