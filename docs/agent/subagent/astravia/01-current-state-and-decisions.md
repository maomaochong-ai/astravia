...
这意味着 Astravia 不需要再造一套简化的 child loop。subagent 应复用同构会话。
...
| 维度 | Codex | Grok Build | Astravia 首版 |
|---|---|---|---|
| 拓扑 | 可递归任务树 | 单层星型 | 单层星型 |
...
原因：主 Agent 本来就负责用户目标、拆分和验收；Astravia 当前没有必须跨兄弟通信或递归分解的产品需求。单层能覆盖并行探索、互斥写集实现和独立 review，同时显著降低恢复与关闭复杂度。
...
Astravia 要同时支持 Windows、普通目录和非 Git 工作区。worktree 应作为独立后续能力，不阻塞核心生命周期。
