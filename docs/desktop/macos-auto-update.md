...
本文是 Astravia Desktop macOS 更新链路的维护手册，与 [`windows-auto-update.md`](./windows-auto-update.md) 平行。两端共用同一套更新源配置、发布脚本、状态机和 CI 编排；差异集中在安装机制与产物形态，本文只展开 macOS 侧。
...
desktop/
  stable/
    latest-mac.yml
    Astravia-<version>-mac.zip              # x64 更新包
    Astravia-<version>-mac.zip.blockmap
    Astravia-<version>.dmg                  # x64 首装包
    Astravia-<version>-arm64-mac.zip        # arm64 更新包
    Astravia-<version>-arm64-mac.zip.blockmap
    Astravia-<version>-arm64.dmg            # arm64 首装包
```
...
内置的 node / python 运行时按 `ASTRAVIA_VENDOR_PLATFORM` **单架构落盘**，一次 electron-builder 调用出不了两套正确产物，因此 arm64 与 x64 必须分两次构建：

```json
"dist:mac:arm64": "cross-env ASTRAVIA_VENDOR_PLATFORM=darwin-arm64 ASTRAVIA_CLI_TARGET_PLATFORMS=darwin-arm64 ... --arch arm64"
"dist:mac:x64":   "cross-env ASTRAVIA_VENDOR_PLATFORM=darwin-x64   ASTRAVIA_CLI_TARGET_PLATFORMS=darwin-x64   ... --arch x64"
```
...
缓存目录由 `app-update.yml` 的 `updaterCacheDirName` 决定，当前是 `astravia-updater`，与 Windows 的 `%LOCALAPPDATA%\astravia-updater\` 同名：

```text
~/Library/Caches/astravia-updater/
  update.zip                                  # 差分基线
  pending/                                    # 下载中的新版本
~/Library/Caches/com.astravia.desktop.ShipIt/    # Squirrel 暂存区
```
...
```bash
mkdir -p ~/Library/Caches/astravia-updater
cp release/Astravia-<installed-version>-arm64-mac.zip ~/Library/Caches/astravia-updater/update.zip
# 核对与线上清单一致
shasum -a 512 -b ~/Library/Caches/astravia-updater/update.zip | awk '{print $1}' | xxd -r -p | base64
```
...
脚本会自动 `source` 两个凭据文件，并直接注入构建期的 `ASTRAVIA_UPDATE_PROVIDER` / `ASTRAVIA_UPDATE_URL`，**因此不依赖 `.env.development` 里有没有配这两项**。

前置校验包括：凭据文件存在且字段完整、钥匙串里有可用签名身份、`ASTRAVIA_R2_PREFIX` 与 `ASTRAVIA_UPDATE_URL` 的末段都等于目标通道、版本号格式合法、**版本严格高于该通道线上已有版本**（同名版本化对象禁止覆盖）。`stable` 额外拒绝 `--version`（正式版本以 `package.json` 为唯一真源）并要求输入版本号二次确认。
...
cp packages/desktop-app/release/Astravia-0.5.62-arm64-mac.zip ~/Library/Caches/astravia-updater/update.zip

# 4. 改点东西，出下一版
scripts/release-mac.sh local --version 0.5.63

# 5. 从终端启动 0.5.62 验证
/Applications/Astravia.app/Contents/MacOS/Astravia
```

两个细节决定这个通道能不能测差分：

- **产物累积不清空**。`~/.astravia/local-updates` 只覆盖 `latest-mac.yml`，旧版本的 zip 与 blockmap 全部保留——差分要读旧版 blockmap（见 5.4）。
...
想临时压缩签名耗时，可以再叠 `ASTRAVIA_SKIP_VENDOR=1`——更新链路不依赖内置 node/python，而它们解压后是几千个待签名的 Mach-O。
...
```bash
source ~/.config/astravia/mac-signing.env
...
ASTRAVIA_UPDATE_URL=https://example.com/astravia/desktop/test
```
...
export ASTRAVIA_R2_BUCKET=astravia-releases
export ASTRAVIA_R2_PREFIX=desktop/test
export ASTRAVIA_UPDATE_URL=https://example.com/astravia/desktop/test
export ASTRAVIA_REQUIRE_MAC_SIGNATURE=1
```

### 7.3 构建测试版本

与 Windows 一样用 `ASTRAVIA_DESKTOP_BUILD_VERSION` 覆盖版本号，不改 `package.json`、不打 tag：

```bash
cd packages/desktop-app
rm -rf release                                  # 残留清单会让发布脚本判定版本不唯一
ASTRAVIA_DESKTOP_BUILD_VERSION=0.5.60 bun run dist:mac:arm64
...
ASTRAVIA_DESKTOP_BUILD_VERSION=0.5.60 bun run dist:mac:x64
ASTRAVIA_REQUIRE_MAC_SIGNATURE=1 bun run verify:updates:mac
...
5. `ASTRAVIA_REQUIRE_MAC_SIGNATURE=1` 时，用 `ditto` 解包每个 ZIP，校验：
   - 顶层有且只有一个 `.app`
   - `CFBundleShortVersionString` 与清单版本一致
   - `CFBundleIdentifier` 是 `com.astravia.desktop`
...
2. 从终端启动以便看日志：`/Applications/Astravia.app/Contents/MacOS/Astravia`
...
tag v<version>
  ...
  -> 读取签名凭据，打开 ASTRAVIA_REQUIRE_MAC_SIGNATURE=1
...
单个 runner 同时只跑一个 job，两个 macOS job 串行，墙钟时间约为单架构的两倍。需要并行可在同一台机器注册第二个 runner 实例（另建目录、同样打 `astravia-mac` 标签）。
...
```bash
launchctl print "gui/$(id -u)/com.astravia.desktop.ShipIt"
```
...
| `state = running` 但版本没变 | ShipIt 在等目标进程退出——检查是不是还有 Astravia 进程活着。 |

**② ShipIt 自己的日志**——Squirrel 只在作业真正 spawn 后才创建这两个文件，**文件不存在本身就是结论**：

```bash
cat ~/Library/Caches/com.astravia.desktop.ShipIt/ShipIt_stderr.log
```
...
Moved bundle contents from ... to file:///Applications/Astravia.app/
Installation completed successfully
Successfully launched application at file:///Applications/Astravia.app/
```

**③ 待安装状态**（安装完成后会被清掉）：

```bash
plutil -p ~/Library/Caches/com.astravia.desktop.ShipIt/ShipItState.plist
```
...
```bash
launchctl kickstart "gui/$(id -u)/com.astravia.desktop.ShipIt"
pkill -f "Astravia.app/Contents/MacOS/Astravia"   # ShipIt 必须等目标退出才替换
```

**别用应用窗口判断应用是否还在跑。** 单实例锁会让新启动的实例退出并把老进程的窗口调出来，看着像「重启了但版本没变」，实际是同一个老进程。用 `pgrep -f "Astravia.app/Contents/MacOS/Astravia"` 看 pid，并和 `launchctl print` 里的 `submitted by Astravia[pid]` 对照。
...
```
Astravia.app/Contents/Resources/vendor/python/cpython-...tar.gz/cpython-...tar/python/bin/python3.13
...
产物里有没签到的嵌套 Mach-O 二进制。`Contents/Resources/` 下带了 `im-gateway`、`cli-app`、`vendor/node`、`vendor/python`、`appshot` 等一堆可执行文件，用 `xcrun notarytool log <submissionId>` 看具体路径。排查时可先 `ASTRAVIA_SKIP_VENDOR=1` 摘掉内置运行时缩小范围。
...
```text
~/.astravia/desktop-app/logs/main/YYYY-MM-DD.log
```
...
打包应用的 stdout 只有从终端启动才看得到：`/Applications/Astravia.app/Contents/MacOS/Astravia`。
...
- [ ] 版本号高于已发布版本。
- [ ] stable 使用 `package.json` 正式版本；test 才使用 `ASTRAVIA_DESKTOP_BUILD_VERSION` 覆盖。
...
- [ ] `ASTRAVIA_REQUIRE_MAC_SIGNATURE=1 bun run verify:updates:mac` 通过。
