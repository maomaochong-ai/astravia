...
describe("package boundary analysis", () => {
	...
	it("detects side-effect and dynamic app imports", () => {
		expect(findPackageBoundaryViolations(libFile, 'import "@astravia/desktop-app";')).toHaveLength(1);
		expect(
			findPackageBoundaryViolations(libFile, 'const app = await import("@astravia/cli-app/runtime");'),
		).toHaveLength(1);
	});

	it("ignores import-looking comments", () => {
		expect(findPackageBoundaryViolations(libFile, '// import app from "@astravia/desktop-app";')).toEqual([]);
	});
	...
	it("allows raw capability ids only in capability definition modules", () => {
		const source = 'const id = "cap.domain.astravia.example.read";';
	...
	it("blocks Desktop globals in ordinary plugins while preserving the built-in workbench exception", () => {
		const source = "window.astravia.fs.readFile(path);";
