...
| 阶段 | 内容 | 说明 |
|---|---|---|
| P7 数据库集成 | 集成 dbx（fork 自 dbx-main 的 MCP 引擎 0.4.61，自有化构建） | 进行中（2026-08-18）：方案 B + 自有化路线（最小化 Rust，引擎二进制经 MCP 接入，不内嵌 WebView/独立进程）已定案并落地——B1 MVP 完成（引擎接入 + AI 对话查库验证）、B2 自有化进行中（B2.1–B2.7/B2.9/B2.10 完成，B2.6-V V1–V6 完成，B2.8 独立排期未启动）；任务清单与状态见 docs/dbx-main-integration-tasks.md |
