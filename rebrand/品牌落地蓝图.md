...
| P7 数据库集成 | 集成 dbx（fork 自 dbx-main 的 MCP 引擎 0.4.61，自有化构建） | 进行中（2026-08-20）：方案 B + 自有化路线（最小化 Rust，引擎二进制经 MCP 接入，不内嵌 WebView/独立进程）已定案并落地——B1 MVP 完成（引擎接入 + AI 对话查库验证）、B2 自有化进行中（B2.1–B2.9 完成，B2.6-V V1–V6 完成，B2.10 W1–W4 完成（2026-08-19，W4-① 感知范围选择性注入、W4-② 环境标记与生产写保护））；B3 已启动（B3.2 数据编辑 + 写操作确认/审计完成、B3.2-R 对齐 dbx 数据编辑与结果工具栏启动（2026-08-20 用户要求：数据编辑不做入口限制，自由 SQL 结果亦可编辑，对齐 dbx 三件套：SQL 可编辑性分析 + 主键/整行等值定位 + 只读原因徽章；结果工具栏参照 dbx 补齐刷新/自动刷新/添加行；批量暂存编辑模型列 R2 后续）、B3.3 自然语言转 SQL few-shot 增强 + 数据洞察成功/失败双场景完成，2026-08-20）；任务清单与状态见 docs/dbx-main-integration-tasks.md |
|---|---|---|
