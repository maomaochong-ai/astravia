...
### 1. 仓库与包名层
- 仓库名：`open-vetta` → `astravia`（GitHub 新仓库 [sikongyue/Astravia](https://github.com/sikongyue/Astravia)；本机目录重命名待执行——被 Vetta Desktop 宿主进程锁定，需退出应用后手动改名）
...
- **origin**：指向 `https://github.com/sikongyue/astravia.git`（用户指定，推送由用户执行）。
- **node_modules**：已被 .gitignore 忽略，不会进入版本库（首次提交仅含源码与配置）。
- **保留**：`rebrand/` 一次性脚本留在仓库中，作为改造记录。

## 七、下一步计划（P0–P6 完成后）

| P7 数据库集成 | 集成 dbx（fork 自 dbx-main 的 MCP 引擎 0.4.61，自有化构建） | 进行中（2026-08-22）：方案 B + 自有化路线（最小化 Rust，引擎二进制经 MCP 接入，不内嵌 WebView/独立进程）已定案并落地——B1 MVP 完成（引擎接入 + AI 对话查库验证）、B2 自有化完成（B2.1–B2.10 完成，B2.6-V V1–V6、B2.6-W W1–W4、B2.9、B2.10 W1–W4 完成；2026-08-22 macOS 平台接入：darwin-arm64/x64 二进制 + fork CI 多平台矩阵）；B3 完成（B3.1 引擎小改 2026-08-23 完成：安全策略定制（执行模式默认最严 + 连接白名单 + 危险 SQL 拦截）+ 执行限制（行数上限/超时可配）+ dbx.db 凭据明文风险评估，`bun run check` 全绿，见 docs/dbx-main-integration-tasks.md；B3.2 数据编辑 + 写操作确认/审计完成；B3.2-R 对齐 dbx 数据编辑与结果工具栏 R1 五项完成（2026-08-22 核实：SQL 可编辑性分析 + 自由 SQL 结果编辑解锁 + 只读原因徽章 + 工具栏刷新/自动刷新/添加行，批量暂存编辑模型列 R2 后续）；B3.3 自然语言转 SQL few-shot 增强 + 数据洞察成功/失败双场景完成；B3.4 Web 版评估可选）；任务清单与状态见 docs/dbx-main-integration-tasks.md |
|---|---|---|
